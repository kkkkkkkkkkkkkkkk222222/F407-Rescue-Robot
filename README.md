...
6. `RETURN_SAFE`：夹紧目标后等待上位机`TYPE=0x18`航向+剩余距离。F407用本地IMU/Location完成绝对航向对准，再锁存编码器定距；剩余距离小于300 mm后线性减速，理论终点速度为300 mm处的一半，不需要`TYPE=0x16`实时位置。定距完成后`TYPE=0x17 P0 bit5 DISTANCE_DONE=1`，供上位机可靠切换到安全区对正，避免T265位置误差导致一直停在NAV。
...
估算底盘前进距离。正式前进轮速比例为`M1=+0.866、M2=-0.866、M3=0`，后退时M1/M2符号相反；横移比例为`M1=-0.5、M2=-0.5、M3=+1.0`，原地旋转仍为三轮同号。Task开局前600 mm直接使用`Motor_MoveDistance()`锁存编码器起点和IMU航向，完成后才展开机构；总1.70 m仍由`Location.path_mm`约束。每次定距启动前确认三轮低速稳定，再以编码器前向解算独立决定剩余距离和3 mm容差制动。导航专用`Motor_MoveDistanceLinear()`保持同一定距闭环，但在内部剩余300 mm内把速度目标线性降到巡航速度的一半。
