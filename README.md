...
`NormalRun`启用完整救援Task，并已按上位机`codex/gamepad-teleop@6c802ec`的APPROACH、目标丢失恢复、HOLD/PAUSE、夹内审核、独立释放、退让、脱困、换道和受控打散命令对齐，同时保留本车机械与现场标定约束。完整流程坐标由`TYPE=0x18/APPROACH_TARGET`发送，仍复用原有1280×1024视觉PID。投送区NAV在D≤50 mm时由F407锁存最终放置方向，D=0补推保持不变；收到`TASK_COMPLETE`后先以编码器和IMU后退0.30 m退出围栏，再按上位机持续发送的RETURN航向H原地调头并依据剩余距离D向前返回中心。正式投送中的普通+核心混装不直接运输：可定位到左右侧时由上位机指定单侧释放、后退复审并仅运送保留的单件；同侧混合或位置不明则双侧释放、后退打散后重新搜索。临时藏堆没有物资种类和数量限制，稳定审核只需确认夹内非空；空爪藏点路线禁用安全区末端锁向与补推。
...
3. `OPEN_CLAW`：到达中心并完成开爪后直接进入`SEARCH`。旧的“前进0.20 m、正反各转一圈、后退0.30 m”无条件开局盲打乱代码已删除；如果上位机通过视觉确认首件绿色物资无法单独取得，仍可发送`DISPERSE_PILE`执行受控强打散。该动作先后退0.30 m蓄力、以1000 mm/s正冲0.50 m，再进行+60°和-120°的边前进边旋转扫拨，最后恢复原航向。两者不是同一流程。
4. `FIND_OBJECT`：每次从舵机3当前命令角以1°/20 ms移到90°，稳定300 ms后以200 mm/s转满360°；未收到新的合法`APPROACH_TARGET`则把舵机3移到120°，再稳定300 ms并旋转一圈，所有阶段都继续接收新目标。两圈约720°完成后F407保持`SEARCH`并重新从90°开始扫描，不再自行按本地Location返回中心。该720°是底盘的本地扫描过程；`6c802ec`上位机可以在扫描期间随时发送合法APPROACH、空爪藏点NAV或DISPERSE接管，普通HOLD不会让SEARCH停止旋转。需要人为或故障恢复期间冻结当前扫描时发送`TYPE=0x18/PAUSE(1)`；该暂停会锁存，恢复SEARCH时发送新SEQ的HOLD。APPROACH丢失停车500 ms后的局部恢复流程保持不变。
5. `GRAB_OBJECT`：SEARCH收到1帧合法单目标便锁定其类别并进入APPROACH。后续只有相同类别的合法新SEQ可以刷新X/Y、距离和丢失计时，其他类别不能中途接管；底盘统一用32位`report_generation`识别新目标帧，使旧`TYPE=0x12`报告和新`TYPE=0x18/APPROACH_TARGET`都能逐帧更新水平PID与舵机3。水平坐标以0.55权重低通，转向PID目标限±175 mm/s并以1000 mm/s²平滑变化，最低有效转向40 mm/s，方向反转必须经过0；误差≤48 px允许最高350 mm/s，48～160 px线性限速，≥160 px最多150 mm/s，避免大偏差下高速走S形。旧帧转向120 ms后开始衰减、300 ms归零，前进仍在250～600 ms平滑停止，连续1200 ms没有原目标才进入REACQ。有效距离≤500/250 mm时仍降到225/125 mm/s。舵机3处于135～139°时把前进限到80 mm/s；仅当同一锁定类别持续有效、该区间保持700 ms且横向误差≤96 px时，才停车并将相机命令移到140°进入抓取观察，作为长物体迟迟不到140°的兜底。
...
8. `STOPPED`：上位机明确`STOP/ABORT`、电机故障、持续位姿无效、出发超时或非法内部状态后保持停车。任务命令短暂断流、暂时出现与当前阶段不匹配的命令，以及静止投送复核期间的相机/规划线程延迟都只停车并保留当前状态，新鲜匹配命令恢复后继续；远程动作暂停期间不累计动作执行超时。连续任务不设置180秒总时长终止，目标重捕获失败返回搜索而不是永久停车。

当前夹爪角度为：收缩左23°/右147°，Touch左80°/右100°，Open左108°/右72°；单侧分离时释放侧完全Open，保留侧额外夹紧10°（保留左70°、保留右110°）。安装机构后必须先断开机构负载标定角度，确认不会顶死舵机。

Task模式LCD保留当前任务状态、上位机X/Y、导航角度、距离D、命令状态和摄像头角度。`T`表示上位机目标行进航向，`L`表示D≤50 mm后采用的固定放置区航向，`A`表示F407实际航向；RETURN中车头目标就是上位机H，不再显示或采用H+180°的倒车航向。`TMO/STALE/PUSH/DONE`分别表示任务帧超时、导航整体无进展、末端慢推和补推完成停车。车辆前进时，D变化≥2 mm、编码器累计位移≥5 mm或IMU航向变化≥1°任一成立都视为仍有进展，三者连续500 ms均不变化才显示`STALE`并停车，避免仅因上位机D整数短时不变而误停。正常载货投送进入D≤20 mm并连续稳定300 ms后只在后台锁存终端超时，原NAV仍按D持续减速前进，D=0或ENTER仍立即执行原流程；若锁存后1.5 s仍未切换，才自动执行现有固定航向、编码器限程的末端慢推。该兜底不作用于RETURN_CENTER和藏点导航。
