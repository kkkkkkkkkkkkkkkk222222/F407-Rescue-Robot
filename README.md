...
2. `START`：主函数初始化时舵机2和舵机4均保持90°；配置连续3帧确认成功后自动启动180秒倒计时，以850 mm/s倒车1秒退出安全区。制动后舵机1恢复85°，先将右爪舵机2转到100°，再将左爪舵机4转到80°形成Touch姿态，然后调用`Motor_TurnAngle(180.0f)`并停车等待5000 ms。
