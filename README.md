...
当前配置为`APP_ENABLE_LOCATION_DEMO=1`、`APP_ENABLE_MOTION_TEST=0`，完整救援任务暂时关闭。上电静止校准IMU并显示`IMU660RC: OK`一秒后，程序假定小车位于4号出发区中心，车头沿右下对角线朝场外，先以300 mm/s倒车3秒，让车尾沿左上方向从两组减速带之间的缝隙进入场内后停车；LCD每100 ms显示估计坐标、航向和场地俯视图。
...
- LCD地图按赛事图绘制四个300×300 mm出发区、上下两个约660×360 mm安全区、中心轴线，以及四个出发区两条场内方向上的全部减速带；亮黄色实心等腰三角形中心代表当前车体位置，白色轮廓提高地图上的对比度，红色顶点代表车头，较长底边代表车尾，绿色轨迹点保留最近48个实测位置。三角形根据IMU航向实时旋转，不是预先绘制的固定图标；IMU失效时填充色会变为红色。
...
输出位于`build/Debug/WWW.elf/.hex/.bin`。当前固件上电后从4号位倒车穿过两组减速带之间的对角缝隙并显示定位地图；先悬空确认倒车时M1正转、M2基本不转、M3反转，同时LCD黄色点向左上移动，再落地低速测试。恢复任务后再用`tools/vision_protocol.py`验证三帧配置、导航超时和抓取流程。S4开闭角、方向、堵转门限、实际轮径、每圈计数和PID参数都必须结合实车数据继续整定。
