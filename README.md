...
7. `DELIVER`：完成安全区入口对正后，持续收到`ENTER_SAFE_ZONE`才允许左右爪同时打开、后退0.30 m并前冲0.55 m。收到`TASK_COMPLETE`后再后退0.45 m；随后执行`RETURN_CENTER`航向+剩余距离和相同线性减速，在距中心约600 mm处重新搜索。上位机看到STM32重新进入SEARCH后清除上一件锁定类别；首件普通物资完成后允许普通、核心、危险和伤员四类单目标进入下一轮。
