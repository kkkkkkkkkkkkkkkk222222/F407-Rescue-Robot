...
2. LCD显示`WAIT S1`后按一次S1，程序先调用`Motor_Move(0, 0, 250)`，通过三轮运动学和编码器速度PID原地旋转3秒。
3. 到3秒时进入本次电机组合测试：底盘测试速度为750 mm/s，对应M1以约-650 mm/s反转、M2停止、M3以约+650 mm/s正转；持续3秒后调用`Motor_Stop()`制动停车并显示`DONE`。该组合只写在测试入口，不改变正式任务的`Motor_Move()`、定距和里程算法。
...
正数朝带舵机的一侧前进、负数后退，单位为米。它记录M2/M3的64位起点，用：

```text
forward_distance = (M3_distance - M2_distance) / sqrt(3)
```

估算底盘前进距离。以带舵机的照片左下方为车头时，M1停止，M2/M3轮速为`-0.866/+0.866`倍。当前驶出出发区使用300 mm/s巡航；减速区取100 mm和全程一半中的较小值，进入减速区后清除巡航积分，并把速度目标从300 mm/s线性降到120 mm/s。PWM仍完全由固定45%基础值和实际编码器速度误差的PI修正得到，不再人为同步压低PWM上限，因此不会因为轮胎低于约50%无法克服静摩擦而在终点前提前停住。进入3 mm容差后主动制动。
