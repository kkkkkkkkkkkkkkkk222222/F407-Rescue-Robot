# 当前固件：四出发区Task与LCD状态监视

当前固件启用完整Task流程，其余独立运动测试、定位演示和舵机扫描均关闭。上电后先停车等待RDK X5通过新PCB“串口1”发送有效配置，随后仍需按下S1才开始车辆动作。该接口使用MCU USART3的PD8 TX、PD9 RX和115200 8N1；RDK X5使用`/dev/ttyS1`，两端TX/RX交叉并共地。
...
Task模式上电时Location保持未配置状态，不再假定4号位；连续3帧赛前配置确认后，程序用`Location_Reset((LocationStart)start_zone)`按1～4号出发区建立对应初始坐标和航向。T265位姿回传尚未加入，运行中仍由本地编码器和IMU推算。
...
Task使用`TYPE=0x16`发送任务状态：状态改变时立即发送，并在任务运行期间每200 ms发送一次。当前`APP_ENABLE_TASK=1`，因此上位机需要正常解析或忽略该帧。联调说明曾把`0x16`预留给后续RDK融合位姿；正式接入融合位姿前，需要由上位机负责人确定新的任务状态类型，届时再统一调整，当前先不改后续流程。
...
该事件只在`RETURN_SAFE`有效。F407把新`SEQ`作为一次独立请求，500 ms内收到后执行850 mm/s车体后退1秒；相同`SEQ`不会重复触发，每次返航最多接受2次。850 mm/s低于当前编码器60 count/10 ms对应的纯前进物理上限，避免三轮同比缩放后自救距离与配置不一致。何时卡住以及是否需要再次自救完全由上位机判断，F407不再根据本地编码器和Location自动触发。是否成功放下仍由F407连续看到3帧夹内为空后确认。
...
上位机在`RETURN_SAFE`中每30～50 ms发送一次导航帧，并持续递增`SEQ`。最近200 ms内的帧会把状态标记为`NAV_FRESH`并允许执行对应动作；超过200 ms未更新就强制停车，收到新的合法匹配帧后才恢复。进入返航后尚未收到匹配命令、目的地不匹配或自救刚结束尚未收到新命令时同样停车。到附近必须连续3帧`P0=00、P1=02`且序号逐帧加1，随后才进入放置流程。
...
5. 停车后舵机1转到65°，夹爪按“舵机4先到180°、舵机2再到0°”的Open顺序释放目标并等待600 ms；摄像头转到0度低头角，等待400 ms让舵机和图像稳定。
...
7. 连续3帧确认夹内为空后才计为送达；摄像头回90度，夹爪回到右100°/左80°的Touch姿态后编码器后退0.50 m，回到`FIND_OBJECT`继续旋转搜索。
8. 若连续3帧确认夹内仍有货物，则重新Touch夹紧目标，后退0.50 m回到`RETURN_SAFE`。剩余货物不合规或电机故障时进入`STOPPED`。
...
2. `START`：舵机1保持55°且爪子保持Retract，等待PA0/S1；启动180秒倒计时，以850 mm/s前进0.70 m。制动后舵机1恢复85°，先将外侧右爪舵机2转到100°，再将左爪舵机4转到80°形成Touch姿态，然后调用`Motor_TurnAngle(180.0f)`并停车等待5000 ms。
3. `FIND_OBJECT`：摄像头保持90°，等待700 ms后原地搜索；累计实际航向转满360°仍未找到目标，就前进0.80 m并从新位置继续搜索。只接近当前阶段允许且分类明确的单个目标。
4. `GRAB_OBJECT`：Task起始时舵机3明确复位到90°；连续3帧确认目标后先保持Touch姿态，按X修正底盘并由舵机3根据目标Y坐标执行视觉PID。水平和摄像头PID都只在新视觉`SEQ`到达时更新，并按实际帧间隔计算I/D；水平控制采用16像素进入、8像素退出的滞环死区，非零修正至少80 mm/s。舵机3达到150°后立即停车，再按“左爪180°、右爪0°”顺序Open；两只爪子全部打开后才进入抓取阶段，保持150°观察并让夹爪回到右100°/左80°的Touch姿态夹持目标，随后进行3帧确认。8秒超时只约束视觉靠近，Open和Touch各自使用3秒动作保护；非法、丢失或失败时保持Touch后再后退重搜。
5. `RETURN_SAFE`：夹紧目标，按目的地匹配且不超过200 ms的`TYPE=14`帧前进、转向、后退或停车；导航过期立即停车。收到新`SEQ`的`TYPE=13、P0=02`后以850 mm/s后退1秒自救；连续3帧确认到区后进入放置。
...
Task模式LCD不再绘制场地图、本地位置、目标坐标、目标距离或`NEAR`。九行状态监视依次显示Task状态与剩余时间、红蓝方/启动区/串口状态、最新帧`TYPE/SEQ`与接收延迟、四类目标数量、`FOUND/GRABBED`、`CLASS_VALID/UNKNOWN`、`CLAW_VIEW`与目的地区、导航方向/到区状态/新鲜度，以及放置阶段/故障码/恢复次数。串口超过250 ms未更新时显示`TMO`并保留最后一次已解析内容，便于现场定位断帧。动态值每100 ms刷新；所有布局和文案封装在`Lcd.c`，`Robot.c`只负责传递同一份原子视觉快照并调度刷新。
...
6. 当前`APP_ENABLE_TASK=1`，所有独立运动测试、定位演示和舵机扫描均为0；上电先执行机构安全初始化并等待配置，只有配置成功后按下S1才开始任务动作。
...
估算底盘前进距离。正式前进轮速比例为`M1=+0.866、M2=-0.866、M3=0`，后退时M1/M2符号相反；横移比例为`M1=-0.5、M2=-0.5、M3=+1.0`，原地旋转仍为三轮同号。调用方同时给出本次最大速度：开局0.70 m使用850 mm/s，搜索后推进0.80 m使用750 mm/s，放置定距仍使用300 mm/s。减速距离按速度相对300 mm/s同比放大，并限制为全程一半，最后线性降到120 mm/s后进入3 mm容差主动制动。
...
输出位于`build/Debug/WWW.elf/.hex/.bin`。当前固件运行完整Task，所有独立测试模式关闭；新PCB“串口1”（USART3）仍独立于Task以100 Hz发送`TYPE=0x15`三路累计编码器计数。烧录后应先架空或断开电机12 V核对配置、舵机顺序和方向，再落地测试；只有配置成功并按下S1才开始任务。可用`tools/vision_protocol.py`验证配置、视觉报告和里程计帧。T265上位机必须使用实测轮径/每圈计数以及正确的相机安装外参；后续融合位姿待上位机协议确定后再接入。
