...
安全区扫障使用独立`CLEAR_SAFE_ZONE=0x13`。新流程P2/P3=0：F407侧放原物资后进入mode42，由上位机发送障碍像素坐标复用视觉PID靠近；mode43使用`AUDIT_SWEEP_PICKUP`三帧非空审核，允许抓取危险、伤员和未知障碍。抓到后F407用保存的二维位姿回到走廊中心，移障并取回原物资，再经mode23复审、mode40和两次ALIGN后ENTER。无障碍时完全不进入扫障；旧80～600 mm距离分支保留兼容。`RELEASE_BOTH`始终是真实双开/mode34；无侧DISPERSE才是20°观察/mode35。
