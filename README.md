...
`NormalRun`启用完整救援Task，并已对齐上位机`codex/gamepad-teleop@676638a`的APPROACH、目标丢失恢复、HOLD、夹内审核、独立释放、退让、脱困、换道和受控打散命令。完整流程坐标由`TYPE=0x18/APPROACH_TARGET`发送，仍复用原有1280×1024视觉PID；返安全区使用动态航向+剩余距离，投送后采用RETURN行进方向的反向车头姿态直接倒车返中。危险、未知、超过3件或伤员混装不会直接投送：F407按上位机指定打开异常侧，另一侧仅保持正常触碰角，退让后摄像头低头重新审核；合法后重新合拢双爪再运输，复审仍失败才双爪全开。临时藏堆和后续空爪复查均复用合法NAV帧；空爪路线禁用安全区末端锁向与补推，到点后回到SEARCH。
...
4. `FIND_OBJECT`：每次从舵机3当前命令角以1°/20 ms移到90°，稳定300 ms后以200 mm/s转满360°；未收到新的合法`APPROACH_TARGET`则把舵机3移到120°，再稳定300 ms并旋转一圈，所有阶段都继续接收新目标。两圈仍无目标时，不再依赖旧`TYPE=0x16`视觉报告计数，也不执行0.80 m盲目前进，而是由本地Location计算到`(0,0)`的航向和距离：超过100 mm则IMU对正后以850 mm/s编码器闭环返中心，已在中心范围内则重新从90°开始两层搜索。APPROACH丢失停车500 ms后也进入同一流程，不再经过140°中转或前进1 m。
...
Task模式LCD保留当前任务状态、上位机X/Y、导航角度、距离D、命令状态和摄像头角度。`T`表示上位机目标行进航向，`L`表示最后100 mm采用的固定放置区航向，RETURN中的`B`表示倒车时采用的车头目标航向（上位机H+180°），`A`表示F407实际航向；`TMO/STALE/PUSH/DONE`分别表示任务帧超时、余量无进展、末端慢推和补推完成停车。正常载货投送进入D≤20 mm并连续稳定300 ms后只在后台锁存终端超时，原NAV仍按D持续减速前进，D=0或ENTER仍立即执行原流程；若锁存后1.5 s仍未切换，才自动执行现有固定航向、编码器限程的末端慢推。该兜底不作用于RETURN_CENTER和藏点导航。
