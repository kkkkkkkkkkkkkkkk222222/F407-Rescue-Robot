...
4. `FIND_OBJECT`：摄像头先保持120°，等待700 ms后以160 mm/s原地转满360°；没有目标就停车，以1°/40 ms慢慢调到90°并再转一圈。两圈均无目标才恢复本轮起始航向，以编码器定距＋IMU航向保持前进0.80 m，再从120°开始新一轮。预计落点距场地边界不足200 mm时先改朝场地中心。舵机扫描、旋转和前进期间均持续接收新视觉帧；进入每轮SEARCH后仍用报告代次隔离开局和上一轮的旧坐标。
...
7. `DELIVER`：到达后仍执行`ALIGN_SAFE_ZONE`，所以车头会在最终30°内以120 mm/s慢速对正红方90°或蓝方270°。持续收到`ENTER_SAFE_ZONE`后左右爪同时打开，直接进入`CHECK`原地保持至少1200 ms，不再后退0.30 m或前冲0.55 m；窗口结束且收到`TASK_COMPLETE`后才后退0.45 m退出。随后执行`RETURN_CENTER`航向+剩余距离，在距中心约600 mm处重新搜索。
