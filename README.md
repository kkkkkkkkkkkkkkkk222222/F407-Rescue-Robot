...
6. `RETURN_SAFE`：夹紧目标后持续接收上位机最新`TYPE=0x18`航向+剩余距离。NAV进入最后300 mm时限速约400 mm/s；首次满足D≤100 mm且车头与上位机最终航向误差≤2°时锁存F407实际航向，最后100 mm忽略上位机H抖动，只保持锁存方向直行，D重新大于150 mm才解除锁定。RETURN仍持续跟随上位机航向，不使用该100 mm锁定；D≤2 mm停车。
...
Task模式LCD保留当前任务状态、上位机X/Y、导航角度、距离D、命令状态和摄像头角度。`T`表示上位机目标航向，`L`表示最后100 mm采用的锁存航向，`A`表示F407实际航向；`TMO/STALE/DONE`分别表示任务帧超时、余量无进展和进入2 mm停车区。
