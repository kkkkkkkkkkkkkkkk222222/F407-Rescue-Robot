...
4. `FIND_OBJECT`：普通搜索先以摄像头120°等待700 ms，再以160 mm/s原地转满360°；没有目标就停车并快速命令舵机3到90°，稳定300 ms后再转一圈。每个视角一整圈至少收到3帧不同SEQ的新视觉报告才允许判定无目标，低帧率时保持当前视角继续旋转。两个视角都确认无目标后才恢复起始航向并前进0.80 m。返中完成后的第一轮直接从90°开始。
5. `GRAB_OBJECT`：合法单目标的X/Y先以0.35权重低通滤波，再分别控制底盘和舵机3；PID最大帧间隔放宽到300 ms，舵机跟踪命令限制为30°/s。明确收到无目标帧时立即停车进入恢复；只是没有新SEQ时冻结舵机，250 ms内保持、250～600 ms平滑降速、600 ms后停车，连续1200 ms仍无帧才进入REACQ。恢复仍为停车500 ms、当前角度慢扫到140°再到90°、90°转一圈和1.00 m换位；任何新合法目标可恢复APPROACH。
