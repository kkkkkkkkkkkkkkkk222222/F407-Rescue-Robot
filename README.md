...
6. `RETURN_SAFE`：夹紧目标后持续接收上位机最新`TYPE=0x18`航向+剩余距离。速度和航向目标都经过每20 ms斜率限制；NAV进入最后300 mm时限速约400 mm/s，RETURN仍在最后300 mm内由500线性降到120 mm/s，≤20 mm停车。命令超过250 ms或余量500 ms无进展便停车；LCD显示`TMO/STALE/DONE`区分通信超时、载荷冻结和到达。
