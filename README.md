...
`NormalRun`启用完整救援Task，并已按上位机`codex/gamepad-teleop`的APPROACH、HOLD/PAUSE、夹内审核、独立释放、退让、脱困、换道和受控打散命令对齐，同时保留本车机械与现场标定约束。完整流程坐标由`TYPE=0x18/APPROACH_TARGET`发送，仍复用原有1280×1024视觉PID。投送区NAV在D≤50 mm时由F407锁存最终放置方向，D=0补推保持不变；收到`TASK_COMPLETE`后先以编码器和IMU后退0.30 m退出围栏，再按上位机持续发送的RETURN航向H原地调头并依据剩余距离D向前返回中心。聚集物块不会在远处直接触发打散：上位机用APPROACH的`CLUSTER_TARGET`标志持续引导，F407以最高300 mm/s靠近，在相机130°附近水平对正后报告`mode=37`，此时才接受`DISPERSE_PILE`。左右归属不明的正式混装改为本地“放下、退让、闭爪前撞、退回撞击起点、重新张爪”，完成后直接回SEARCH，不再执行没有货物的夹内复审；临时藏堆的`RELEASE_BOTH`仍始终双开。
...
3. `OPEN_CLAW`：到达中心并完成开爪后直接进入`SEARCH`。旧的无条件开局盲打乱已经删除。上位机确认目标聚集时，先持续发送带`CLUSTER_TARGET(bit5)`的`APPROACH_TARGET`；F407以最高300 mm/s沿用视觉PID靠近，相机到130°后只原地水平对正，X误差≤40 px时停车并上报`mode=37/DISPERSE_READY`。上位机随后发送`DISPERSE_PILE`，F407在接受该命令的瞬间锁存IMU航向作为0°，先把双爪同时闭到Touch（左80°、右100°），再以400 mm/s专用转速分别闭环到`+45°、-45°、+90°、-90°`，不再回正，最后沿当前车尾方向以550 mm/s后退0.30 m。动作完成才上报`mode=35`。
...
5. `GRAB_OBJECT`：SEARCH收到1帧合法单目标便锁定类别并进入APPROACH，同类新SEQ才更新X/Y。正常靠近和中距离速度均为350 mm/s，近距有效距离≤250 mm时为125 mm/s。相机到125°立即停止平移，只原地对正到横向误差≤24 px；随后相机到140°，锁存IMU航向并以350 mm/s前进最多300 mm等待目标重新入镜。重获且误差≤96 px则进入抓取观察；未重获则停车，以1°/40 ms慢抬相机到90°，途中重获立即继续APPROACH，到90°仍没有才回SEARCH。NormalRun不再按250 ms帧龄自行停车，控制改变由上位机明确命令负责。
6. `RETURN_SAFE`：夹紧目标后持续接收上位机最新`TYPE=0x18`航向+剩余距离。NAV进入最后300 mm时限速约400 mm/s；首次进入D≤50 mm时，无论当前车头方向如何，F407都立即停车并锁存放置区法向；第一次投送采用红方右偏10°到80°、蓝方左偏10°到280°的补偿，第一次收到`TASK_COMPLETE`后，后续恢复红方90°/蓝方270°；误差≤1.5°后才推进末段。行进修正死区为1°，动态偏差达到4°会停车重对。此后忽略上位机H变化，只保持固定方向；D重新大于100 mm解除锁定。NAV/RETURN携带的红蓝标志必须与锁存颜色一致。首次收到NAV的D=0后，以200 mm/s继续慢推，保证至少1.5 s、最长2.0 s；编码器440 mm上限只有满1.5 s后才能触发，随后停车并置`DISTANCE_DONE`；提前到达的ENTER会被确认，但F407仍会先锁存放置方向、完成对正和末端慢推，再执行张爪。RETURN不使用末端慢推。
7. `DELIVER`：安全区车头对正已经删除。上位机确认到达后直接发送`ENTER_SAFE_ZONE`，F407从NAV同时打开左右爪；完全张开后摄像头转到120°并稳定300 ms，再进入`CHECK`原地保持至少1200 ms，让上位机完成“区外→区内”视觉确认。不会进入ALIGN，也不执行碰撞。收到`TASK_COMPLETE`后先进入`EXITSAFE(mode16)`，以400 mm/s编码器闭环后退0.30 m并保持退出时IMU航向；随后进入`FACE_FIELD_CENTER(mode17)`等待上位机持续发送`RETURN_CENTER`行进方向H和剩余距离D。F407先把车头转到H（3°内完成），再最高800 mm/s向前返中，途中连续小幅修正，偏差达到8°才停车重对，D=0后进入90°/120°SEARCH。实际返中终点完全由上位机发送的D决定。
...
当前夹爪角度为：收缩左23°/右147°，Touch左80°/右100°，Open左108°/右72°；单侧分离时释放侧完全Open，保留侧额外夹紧10°（保留左70°、保留右110°）。安装机构后必须先断开机构负载标定角度，确认不会顶死舵机。

左右归属不明时的第一次正式`RELEASE_BOTH`是特殊撞分支：双爪先全开让货物留在原处，后退0.40 m；双爪回Touch后以700 mm/s前进0.40 m撞散，再以450 mm/s后退0.40 m回到撞击起点；随后双爪重新完全打开并上报`mode=34`。这时爪内没有保留物资，不再进入WATCH复审；上位机发送HOLD后F407直接进入SEARCH，相机自动回90°。

Task模式LCD保留当前任务状态、上位机X/Y、导航角度、距离D、命令状态和摄像头角度。`T`表示上位机目标行进航向，`L`表示D≤50 mm后采用的固定放置区航向，`A`表示F407实际航向；RETURN中车头目标就是上位机H。任务帧不再显示`TMO`，收到合法帧后持续显示并执行最后命令；`STALE/PUSH/DONE`分别表示导航整体无进展、末端慢推和补推完成停车。
