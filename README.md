...
`NormalRun`启用完整救援Task，并已按上位机`codex/gamepad-teleop@6c802ec`的APPROACH、HOLD/PAUSE、夹内审核、独立释放、退让、脱困、换道和受控打散命令对齐，同时保留本车机械与现场标定约束。完整流程坐标由`TYPE=0x18/APPROACH_TARGET`发送，仍复用原有1280×1024视觉PID。投送区NAV在D≤50 mm时由F407锁存最终放置方向，D=0补推保持不变；收到`TASK_COMPLETE`后先以编码器和IMU后退0.30 m退出围栏，再按上位机持续发送的RETURN航向H原地调头并依据剩余距离D向前返回中心。正式投送中的普通+核心混装不直接运输：可定位到左右侧时按指定侧释放；左右归属不明时第一次`RELEASE_BOTH`固定左开右正常闭合，后退复审，仍非法才真正双开。临时藏堆没有物资种类和数量限制，且`RELEASE_BOTH`始终双开；空爪藏点路线禁用安全区末端锁向与补推。
...
3. `OPEN_CLAW`：到达中心并完成开爪后直接进入`SEARCH`。旧的无条件开局盲打乱已经删除；如果上位机确认首件绿色物资无法单独取得，仍可发送`DISPERSE_PILE`。F407先把双爪合到Touch，以700 mm/s正冲0.50 m；再完全开爪，按相对初始航向`+45°、-45°、+60°、-60°、0°`分段摆头，最后以650 mm/s倒退0.50 m。动作完成才上报`mode=35`。
