...
`NormalRun`启用完整救援Task，并已对照上位机`codex/gamepad-teleop@f7f4793`的APPROACH、HOLD/PAUSE、夹内审核、独立释放、退让、脱困、换道和新版安全区投送命令。正式投送先用`STAGE_ONLY NAV`到上位机设定的安全区前600 mm预备点并以`mode=10 + DISTANCE_DONE`停车，同时把相机抬到120°；该路由兼容上位机当前的精简flags（`VALID|DISTANCE_VALID|STAGE_ONLY|阵营位`）以及旧版完整方向flags，但普通NAV、藏堆NAV和RETURN仍必须携带完整方向flags。随后依次执行90°/270°定位ALIGN和冻结框像素误差视觉ALIGN，两次完成均上报`mode=11`。ENTER不再携带距离：F407首次取得有效位姿时锁存编码器起点和最终航向，考虑旋转中心到前挡板约200 mm，先按400 mm本地距离估算完成400→200 mm/s接近，在累计287 mm处切入300 mm/s、200 mm最终补推；随后停车、张爪、相机120°稳定300 ms才上报mode15。

普通目标在125°完成水平对正后把舵机3转到140°并停车稳定500 ms，随后不再要求原目标重新入镜，直接进入`WATCH/mode21`并置`CLAW_VISIBLE=1`。底盘保持锁存IMU航向，以180 mm/s慢速爬行；F407只把`audit_id`不同、内容一致的审核计作两张视觉帧，连续2帧非空审核后立即停车并允许GRAB，不要求STABLE位。累计最多500 mm仍未确认夹内物体则进入目标丢失恢复。单侧释放后的复审保持原地。聚集目标靠近时保持双爪完全打开，不再依据相机角度或目标Y坐标执行0.12 m后退和Touch闭爪。相机到130°并水平对正后先转到140°并稳定500 ms，再上报`mode=38`、保持IMU航向以200 mm/s爬行；聚集审核仍沿用既有STABLE/mode37握手。第一次带侧曲线分离仅把保留侧相对Touch加紧15°，140°复审后仍有多物体或混装时，后续分离改用25°强保持；稳定空爪审核会完全张爪并直接返回90° SEARCH。
...
7. 收到1帧合法的夹内为空报告后计为送达；摄像头回90度，夹爪回到右102°/左78°的Touch姿态后编码器后退0.50 m，回到`FIND_OBJECT`继续旋转搜索。
...
3. `OPEN_CLAW`：到达中心并完成开爪后直接进入`SEARCH`。上位机确认目标聚集时，持续发送带`CLUSTER_TARGET(bit5)`的`APPROACH_TARGET`；F407保持双爪完全打开并以最高300 mm/s靠近，不再先后退或把双爪闭到Touch。相机到130°并水平对正后转到140°，以200 mm/s慢爬，只有新的稳定非空夹爪ROI审核才能进入`mode=37/DISPERSE_READY`。需要分离时，上位机优先保留含绿色的一侧；两侧都含绿色时保留数量较少的一侧，平局再比较锁定目标、轨迹稳定度和距离，然后发送带`SIDE_VALID(bit6)`及可选`TARGET_RIGHT(bit7)`的`DISPERSE_PILE`。第一次曲线分离把保留侧相对Touch加紧15°（左63°或右117°），另一侧完全打开并沿0.30 m镜像曲线退出；相机140°复审后若仍为多物体或混装，下一次分离自动改用25°强保持（左53°或右127°）。复审为空则双爪完全打开并直接回到90° SEARCH。若无法可靠分侧，上位机发送不带bit6/bit7的`DISPERSE_PILE`；F407只原地转12°并保持140°视角，再上报`mode=35`等待新审核，不再正撞整堆或猜测左右侧。
4. `FIND_OBJECT`：每次从舵机3当前命令角以1°/20 ms移到90°，稳定300 ms后以200 mm/s转满360°；未收到新的合法`APPROACH_TARGET`则把舵机3移到120°，再稳定300 ms并旋转一圈，所有阶段都继续接收新目标。两圈约720°完成后F407保持`SEARCH`并重新从90°开始扫描，不再自行按本地Location返回中心。上位机可用普通APPROACH接管单目标，或用带`CLUSTER_TARGET`的APPROACH进入聚集目标靠近；不能再从远处直接发DISPERSE。普通HOLD不会让SEARCH停止旋转，显式PAUSE才冻结。
...
7. `DELIVER`：ENTER要求`DRIVE_STRAIGHT=1、DISTANCE_VALID=0、P2..P5=0`，视觉ALIGN成功时置bit6且P6/P7=0，定位降级时置`USE_FINAL_HEADING`并发送红9000/蓝27000。F407保持ALIGN锁存航向，在第一次有效LocationPose只锁存一次编码器起点；重复ENTER只ACK。底盘按有效接近距离400 mm估算400→200 mm/s速度，累计287 mm后以300 mm/s再补推200 mm，最长1200 ms。随后立即停车，同时打开左右爪、摄像头转到120°并稳定300 ms，再进入`CHECK(mode15)`原地保持至少1200 ms。收到`TASK_COMPLETE`后执行`EXITSAFE(mode16)`后退0.30 m和`FACE_FIELD_CENTER(mode17)`返中；返中全程保持120°，到中心停车后明确经过`120°稳定→90°`，丢弃移动相机期间的旧视觉帧，取得90°新帧后才允许进入下一次APPROACH。
8. `STOPPED`：上位机明确`STOP/ABORT`、IMU/位姿等运动前提失效、出发或远程动作超时、或非法内部状态后保持停车。NormalRun不再根据单轮编码器方向、单轮零速或定距短时无进展自动锁存`TASK_FAULT_MOTOR`，这些运行进展由上位机融合定位监控；MotionDebug和Gamepad等无任务上位机兜底的独立固件仍保留本地电机保护。任务命令短暂断流、暂时出现与当前阶段不匹配的命令，以及静止投送复核期间的相机/规划线程延迟都只停车并保留当前状态，新鲜匹配命令恢复后继续；远程动作暂停期间不累计动作执行超时。连续任务不设置180秒总时长终止，目标重捕获失败返回搜索而不是永久停车。

当前夹爪角度为：收缩左23°/右147°，Touch左78°/右102°，Open左108°/右72°。聚集物第一次曲线分离使用15°柔性保持：释放左侧为左108°/右117°，释放右侧为左63°/右72°。140°复审后仍有多物体或普通爪内错误物资分离使用25°强保持：释放左侧为左108°/右127°，释放右侧为左53°/右72°。随后的YIELD按命令距离连续执行后退、向保留侧横移并向释放侧转头的镜像弧线，完成后相机到140°复审；LCD显示`ACT:YIELD CURVE/CAM/WAIT/DONE`。安装机构后必须先断开机构负载标定角度，确认不会顶死舵机。

左右归属不明时不再撞击或双开整堆。无`SIDE_VALID`的`DISPERSE_PILE`以及旧上位机在首次不明侧审核中发送的`RELEASE_BOTH`，都会转换为12°原地观察转向；相机保持140°并稳定300 ms后上报`mode=35`，等待一组新的夹爪ROI审核。观察转向本身不计为一次曲线分离，因此之后第一次可靠分侧仍使用15°柔性保持；真正完成过一次曲线后复审仍不合法，才切换到25°强保持。仍然无法分侧时上位机可再观察一次，但必须限制次数，不能无限转向。稳定空爪审核会清除当前批次、完全张爪并直接回到90° SEARCH。真正的临时藏点释放和最终放弃批次仍使用双爪完全打开，不受本规则影响。F407接受连续2个内容相同的非STABLE审核，或1个显式STABLE审核；当前上位机策略应使用进入审核阶段后的1个新鲜`frame_sequence`生成显式STABLE结果。

去放置区的`STAGE_ONLY NAV`除正常`D=0`到达外，还带有分段编码器兜底。F407在该段首帧锁存编码器起点和初始D，并把最大允许行程设为`初始D+100 mm`；当本段行程达到该上限且上位机剩余距离`D<=30 mm`时，同样停车、把相机抬到120°并上报`mode=10 + DISTANCE_DONE`。若已经达到编码器上限但D仍大于30 mm，则停车等待上位机修正D，禁止继续盲跑。该条件只完成预备点NAV，后续仍须完成ALIGN、ENTER、张爪投送后才能返中。

Task模式LCD保留当前任务状态、上位机X/Y、导航角度、距离D、命令状态和摄像头角度。NAV/RETURN中`T`表示上位机目标航向、`L`表示F407已锁存航向、`A`表示实际航向；ALIGN显示目标航向、实际航向和第二次修正的像素误差。`PUSH/DONE`分别表示最终35 mm本地补推和投送运动完成。
...
估算底盘前进距离。正式前进轮速比例为`M1=+0.866、M2=-0.866、M3=0`，后退时M1/M2符号相反；横移比例为`M1=-0.5、M2=-0.5、M3=+1.0`，原地旋转仍为三轮同号。开局600 mm和搜索换位仍使用`Motor_MoveDistance()`完成本地编码器定距；NAV与RETURN已改用`Motor_Move()`执行上位机持续更新的航向和剩余距离，不再使用`Motor_MoveDistanceLinear()`锁存整段。轮胎打滑造成的位置误差由上位机下一帧融合余量修正。
