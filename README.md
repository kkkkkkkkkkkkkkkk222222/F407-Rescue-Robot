...
当前固件启用完整Task流程，其余独立运动测试、定位演示和舵机扫描均关闭。上电后先停车等待RDK X5通过新PCB“串口1”发送有效配置；收到1帧合法配置后自动开始车辆动作，不再等待按键。该接口使用MCU USART3的PD8 TX、PD9 RX和115200 8N1；RDK X5使用`/dev/ttyS1`，两端TX/RX交叉并共地。
...
6. 当前`APP_ENABLE_TASK=1`，所有独立运动测试、定位演示和舵机扫描均为0；上电先执行机构安全初始化并等待配置，收到1帧合法配置后自动开始任务动作。
...
输出位于`build/Debug/WWW.elf/.hex/.bin`。当前固件运行完整Task，所有独立测试模式关闭；新PCB“串口1”（USART3）仍独立于Task以100 Hz发送`TYPE=0x15`三路累计编码器计数。烧录后应先架空或断开电机12 V核对配置、舵机顺序和方向，再落地测试；上位机发送1帧有效配置后任务会自动启动。可用`tools/vision_protocol.py`验证配置、视觉报告和里程计帧。T265上位机必须使用实测轮径/每圈计数以及正确的相机安装外参；后续融合位姿待上位机协议确定后再接入。
