...
2. `START`：舵机1保持55°且爪子保持Retract，等待PA0/S1；按键后启动180秒倒计时，以850 mm/s倒车1秒退出安全区。制动后舵机1恢复85°，先将外侧右爪舵机2转到100°，再将左爪舵机4转到80°形成Touch姿态，然后调用`Motor_TurnAngle(180.0f)`并停车等待5000 ms。
...
接通电机12 V前应确认三轮与编码器方向正确，并在车后方预留至少1 m安全空间；只有配置成功后按下PA0/S1才开始180秒倒计时并以850 mm/s倒车1秒。
