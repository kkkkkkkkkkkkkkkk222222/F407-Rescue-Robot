...
当前只启用完整`Task`，所有测试模式和独立`CenteringTask`均关闭，并已适配`danmo-teng/shijue_fangan@bb8a138`的100 Hz任务心跳与动态导航协议。视觉坐标为原生1280×1024、中心`(640,512)`；正常任务接收`TYPE=0x11/0x12/0x18`，发送`TYPE=0x15/0x17`。返安全区和靠近中心持续采用RDK按最新融合位置发送的绝对航向+剩余距离，F407负责IMU转向、速度限制、载荷停滞与失联停车，不再锁存第一帧距离自行完成整段。
...
6. `RETURN_SAFE`：夹紧目标后持续接收上位机最新`TYPE=0x18`航向+剩余距离。每帧剩余距离直接更新速度目标，每帧绝对航向直接更新转向；剩余300 mm内NAV从800降到160 mm/s、RETURN从500降到120 mm/s，≤20 mm停车。命令超过250 ms或车辆前进时余量500 ms没有变化便停车等待，不使用旧距离继续盲跑。
