...
`NormalRun`启用完整救援Task，并已对照上位机`codex/gamepad-teleop@952853c`的APPROACH、HOLD/PAUSE、夹内审核、独立释放、退让、脱困、换道和新版安全区投送命令。正式投送先用`STAGE_ONLY NAV`到上位机当前设定的安全区前600 mm预备点并以`mode=10 + DISTANCE_DONE`停车，同时把相机抬到120°；该路由兼容上位机当前的精简flags（`VALID|DISTANCE_VALID|STAGE_ONLY|阵营位`）以及旧版完整方向flags，但普通NAV、藏堆NAV和RETURN仍必须携带完整方向flags。随后依次执行90°/270°定位ALIGN和冻结框像素误差视觉ALIGN，两次完成均上报`mode=11`。ENTER阶段锁存最终航向，只使用围栏法向剩余距离控制400→200 mm/s减速；到约113 mm机构接触位置后忽略定位变化，以300 mm/s编码器补推200 mm，再张爪并上报mode15。上位机仍需按本文最新无侧观察转向语义更新后才能完整匹配本固件。
...
3. `OPEN_CLAW`：到达中心并完成开爪后直接进入`SEARCH`。上位机确认目标聚集时，持续发送带`CLUSTER_TARGET(bit5)`的`APPROACH_TARGET`；F407先停车把双爪闭到Touch（左80°、右100°）再以最高300 mm/s靠近。相机到130°且X误差≤40 px后上报`mode=37/DISPERSE_READY`。需要分离时，上位机优先保留含绿色的一侧；两侧都含绿色时保留数量较少的一侧，平局再比较锁定目标、轨迹稳定度和距离，然后发送带`SIDE_VALID(bit6)`及可选`TARGET_RIGHT(bit7)`的`DISPERSE_PILE`。F407校验保留侧非空，将保留侧加紧15°（左65°或右115°）、完全打开另一侧并沿0.30 m镜像曲线退出，随后相机140°复审并上报`mode=35`。若无法可靠分侧，上位机发送不带bit6/bit7的`DISPERSE_PILE`；F407只原地转12°并保持140°视角，再上报`mode=35`等待新审核，不再正撞整堆或猜测左右侧。
...
7. `DELIVER`：ENTER只接收围栏法向剩余距离，不再接收动态车头方向。F407保持ALIGN锁存航向，D从300 mm内按400→200 mm/s平滑减速；到约113 mm机构接触位置后忽略定位变化，以300 mm/s编码器补推200 mm，最长1200 ms。随后同时打开左右爪、摄像头转到120°并稳定300 ms，再进入`CHECK(mode15)`原地保持至少1200 ms。收到`TASK_COMPLETE`后执行`EXITSAFE(mode16)`后退0.30 m和`FACE_FIELD_CENTER(mode17)`返中；返中全程保持120°，到中心停车后明确经过`120°稳定→90°`，丢弃移动相机期间的旧视觉帧，取得90°新帧后才允许进入下一次APPROACH。
...
当前夹爪角度为：收缩左23°/右147°，Touch左80°/右100°，Open左108°/右72°。单侧分离时释放侧完全Open，保留侧相对Touch加紧15°：释放左侧为左108°/右115°，释放右侧为左65°/右72°。随后的YIELD按命令距离连续执行后退、向保留侧横移并向释放侧转头的镜像弧线，完成后相机到140°复审；LCD显示`ACT:YIELD CURVE/CAM/WAIT/DONE`。安装机构后必须先断开机构负载标定角度，确认不会顶死舵机。

左右归属不明时不再撞击或双开整堆。无`SIDE_VALID`的`DISPERSE_PILE`以及旧上位机在首次不明侧审核中发送的`RELEASE_BOTH`，都会转换为12°原地观察转向；相机保持140°并稳定300 ms后上报`mode=35`，等待一组新的夹爪ROI审核。新审核能够分侧后，再按普通`SIDE_VALID`曲线分离；仍然无法分侧时上位机可再观察一次，但必须限制次数，不能无限转向。真正的临时藏点释放和最终放弃批次仍使用双爪完全打开，不受本规则影响。
