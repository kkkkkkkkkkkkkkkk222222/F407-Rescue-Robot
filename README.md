...
3. 到3秒时直接切换为`Motor_Move(600, 0, 0)`，以带舵机的一侧为车头：照片左下M1停止，左上M2与右侧M3反向转动，持续3秒后调用`Motor_Stop()`制动停车并显示`DONE`。当前两个速度分别是最初164和300 mm/s测试值的两倍。
...
```text
forward_distance = (M2_distance - M3_distance) / sqrt(3)
```

估算底盘前进距离。以带舵机的一侧为车头时，M1停止，M2/M3轮速为`+0.866/-0.866`倍。当前驶出出发区使用300 mm/s巡航；减速区取100 mm和全程一半中的较小值，进入减速区后清除巡航积分，并把速度目标从300 mm/s线性降到120 mm/s。PWM仍完全由固定45%基础值和实际编码器速度误差的PI修正得到，不再人为同步压低PWM上限，因此不会因为轮胎低于约50%无法克服静摩擦而在终点前提前停住。进入3 mm容差后主动制动。
