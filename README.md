...
当前配置为`APP_ENABLE_LOCATION_DEMO=1`、`APP_ENABLE_MOTION_TEST=0`，完整救援任务暂时关闭。上电静止校准IMU并显示`IMU660RC: OK`一秒后，程序假定小车位于4号出发区中心，沿车头方向以300 mm/s直行3秒，越过左侧三根减速带后停车；LCD每100 ms显示估计坐标、航向和场地俯视图。测试时必须把车头（舵机方向）朝向4号位左侧的场地内部，否则地图轨迹会与实车相反。

## 定位坐标系与算法

- 以3000×3000 mm场地中心为原点，向图纸右侧为`+X`，向图纸上方为`+Y`，航向角从`+X`逆时针增加。4号出发区中心约为`(1350,-1350) mm`，初始航向为180°；直行3秒的理想终点约为`(450,-1350) mm`，实际LCD位置以编码器测量为准。
...
- LCD地图按赛事图绘制四个300×300 mm出发区、上下两个约660×360 mm安全区、中心轴线和4号位三根减速带；黄色点代表车体中心，白线代表车头方向。LCD有矩形填充和文字接口，本次补充了内部短线绘制，因此不需要更换屏幕驱动。
...
输出位于`build/Debug/WWW.elf/.hex/.bin`。当前固件上电后从4号位向场地左侧直行3秒并显示定位地图；先悬空确认M1反转、M2基本不转、M3正转及LCD点向左移动，再落地低速测试。恢复任务后再用`tools/vision_protocol.py`验证三帧配置、导航超时和抓取流程。S4开闭角、方向、堵转门限、实际轮径、每圈计数和PID参数都必须结合实车数据继续整定。
