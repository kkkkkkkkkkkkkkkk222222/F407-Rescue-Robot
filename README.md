...
`NormalRun`启用完整救援Task，并已按上位机`codex/gamepad-teleop@5b6164f`的APPROACH、HOLD/PAUSE、夹内审核、独立释放、退让、脱困、换道、受控打散和新版安全区投送命令对齐。正式投送先用`STAGE_ONLY NAV`到上位机当前设定的安全区前600 mm预备点并以`mode=10 + DISTANCE_DONE`停车，同时把相机抬到120°；再依次执行90°/270°定位ALIGN和冻结框像素误差视觉ALIGN，两次完成均上报`mode=11`。ENTER阶段锁存最终航向，只使用围栏法向剩余距离控制400→200 mm/s减速；到约113 mm机构接触位置后忽略定位变化，以300 mm/s编码器补推200 mm，再张爪并上报mode15。收到`TASK_COMPLETE`后的0.30 m退出围栏和RETURN返中流程不变。
...
3. `OPEN_CLAW`：到达中心并完成开爪后直接进入`SEARCH`。上位机确认目标聚集时，持续发送带`CLUSTER_TARGET(bit5)`的`APPROACH_TARGET`；F407先停车把双爪闭到Touch（左80°、右100°）再以最高300 mm/s靠近。相机到130°且X误差≤40 px后上报`mode=37/DISPERSE_READY`。需要分离且两侧均为合规物资时，上位机优先保留绿色所在侧，否则保留非空且数量较少的一侧，再发送带`SIDE_VALID(bit6)`及可选`TARGET_RIGHT(bit7)`的`DISPERSE_PILE`。F407校验保留侧非空，将保留侧加紧15°（左65°或右115°）、完全打开另一侧并沿0.30 m镜像曲线退出，随后相机140°复审并上报`mode=35`。若无法可靠分侧，则发送不带bit6/bit7的`DISPERSE_PILE`，F407在当前位置闭到Touch并立即以1000 mm/s前撞0.25 m，再以500 mm/s后退0.25 m、张爪，上报`mode=34`约500 ms后自动进入90° SEARCH；500 ms内新的APPROACH仍可直接接管。
...
左右归属不明时，抓取前的`DISPERSE_PILE`（不带bit6/bit7）和抓取后第一次正式`RELEASE_BOTH`共用特殊撞分支：不再先张爪后退蓄力，而是在当前近场位置把双爪闭到Touch作为碰撞面，随后以1000 mm/s恒速定距前撞0.25 m，再以500 mm/s后退0.25 m离开物块，完全张爪并上报`mode=34`。F407保持该完成态500 ms供上位机确认，然后自动进入90° SEARCH，避免上位机无命令重选窗口与下位机等待HOLD互锁；在此窗口内收到新的普通或聚集APPROACH仍可直接接管。
