...
7. `DELIVER`：ENTER只接收围栏法向剩余距离，不再接收动态车头方向。F407保持ALIGN锁存航向，D从300 mm内按400→200 mm/s平滑减速；到约113 mm机构接触位置后忽略定位变化，以250 mm/s编码器补推50 mm。随后同时打开左右爪、摄像头转到120°并稳定300 ms，再进入`CHECK(mode15)`原地保持至少1200 ms。收到`TASK_COMPLETE`后原有`EXITSAFE(mode16)`后退0.30 m与`FACE_FIELD_CENTER(mode17)`返中流程不变。
