...
2. 小车沿180°方向平滑加速到750 mm/s，即朝舵机反方向倒车；达到目标速度矢量后再开始计算3秒倒车时间，倒车期间IMU航向PI抑制意外偏转。测试时给小车留出足够空间并随时准备断开电机电源。
3. 倒车结束后主动制动并等待100 ms，随后调用`Motor_TurnAngle(180.0f)`原地旋转180°；剩余30°时自动减速，进入1°停止容差后制动停车。
4. 第一次旋转完成后调用`Motor_Stop()`清除定角度完成锁存，停稳100 ms，再完整重复一次“倒车3秒→旋转180°”；第二次旋转完成后保持停车。
...
完成或故障状态会锁存，处理结果后调用`Motor_Stop()`回到空闲状态，才能开始下一次定角度动作。当前测试把“倒车3秒并制动100 ms→`Motor_TurnAngle(180.0f)`”完整执行两轮；重复次数由`APP_MOTION_TEST_REPEAT_COUNT`设置，旋转角度由`APP_MOTION_TEST_TURN_DEG`设置，不需要新增重复函数。
