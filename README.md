...
安全区扫障使用独立`CLEAR_SAFE_ZONE=0x13`，不会复用SEARCH/APPROACH阶段的CHANGE_LANE。它只在围栏前600 mm预备点完成第二次视觉ALIGN后接受：普通/核心把当前货物右移150 mm暂放，伤员左移150 mm暂放；随后回中抓取正前方障碍、放到相反侧、回到预备点重新夹回原货物并进入mode23复审。合法后上报mode40，上位机必须重新执行两次ALIGN后才可ENTER。`RELEASE_BOTH`现在始终是真实双开/mode34；无侧DISPERSE才是12°观察/mode35。

定位距3 m×3 m地图任一边≤300 mm时，下位机自主取消当前动作、张爪并转向场地中心，期间上报mode41，随后回mode3重新SEARCH；仅ENTER主动推送阶段放宽到距边50 mm，避免定位进入安全区时误触发。SEARCH完成120°和90°两圈后不会原地静止，而会以150 mm/s低速补扫，防止上位机T265少累计几度后无法发送RETURN。
...
左右归属不明时不再撞击或双开整堆。无`SIDE_VALID`的`DISPERSE_PILE`以及旧上位机在首次不明侧审核中发送的`RELEASE_BOTH`，都会转换为12°原地观察转向；相机保持140°并稳定300 ms后上报`mode=35`，等待新的夹爪ROI审核。观察之后及任何后续带侧曲线仍统一使用15°保持，不升级为25°。稳定空爪审核会清除批次、完全张爪并回SEARCH；25°只用于普通单侧释放。
