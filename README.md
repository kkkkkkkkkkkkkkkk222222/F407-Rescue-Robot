...
`NormalRun`启用完整救援Task，并已按上位机`codex/gamepad-teleop@775bef3`的APPROACH、HOLD/PAUSE、夹内审核、独立释放、退让、脱困、换道、受控打散和新版安全区投送命令对齐。正式投送先用`STAGE_ONLY NAV`到安全区前400 mm预备点并以`mode=10 + DISTANCE_DONE`停车；再依次执行90°/270°定位ALIGN和冻结框像素误差视觉ALIGN，两次完成均上报`mode=11`。ENTER阶段锁存最终航向，只使用围栏法向剩余距离控制400→200 mm/s减速；到约113 mm机构接触位置后忽略定位变化，以300 mm/s编码器补推200 mm，再张爪、抬相机到120°并上报mode15。收到`TASK_COMPLETE`后的0.30 m退出围栏和RETURN返中流程不变。

普通目标在舵机3命令达到140°后停车稳定500 ms，并丢弃稳定前的旧视觉帧；随后以350 mm/s重新观察。第一帧重新出现且X误差≤96 px时进入WATCH并置`CLAW_VISIBLE=1`，但不会立即停车，而是保持IMU航向以150 mm/s慢速爬行；收到上位机带STABLE的非空夹内审核后立即停车等待GRAB。从140°稳定点累计最多前进300 mm仍未确认夹内物体则进入REACQ，不增加Y坐标或连续目标帧门槛。单侧释放后的复审保持原地。聚集目标则在靠近前先把双爪闭到Touch，避免张爪将待打散物资收入爪内；若近距离才切换为聚集目标，会先张爪后退0.12 m清空爪内再闭爪靠近。相机到130°并水平对正后不会直接允许打散，而是先转到140°并稳定500 ms，再上报`mode=38`、保持IMU航向以200 mm/s爬行；只有收到新的稳定非空`CARGO_AUDIT`才停车并进入`mode=37`。审核为单件合法物资时接受`GRAB_CONFIRMED`直接抓取，多件或混装时接受`DISPERSE_PILE`执行分离；累计300 mm或2000 ms仍未确认夹内物资则张爪返回SEARCH。
...
3. `OPEN_CLAW`：到达中心并完成开爪后直接进入`SEARCH`。上位机确认目标聚集时，持续发送带`CLUSTER_TARGET(bit5)`的`APPROACH_TARGET`；F407先停车把双爪闭到Touch（左80°、右100°）再以最高300 mm/s靠近。相机到130°且X误差≤40 px后上报`mode=37/DISPERSE_READY`。上位机若能判断目标位于左右哪侧，发送带`SIDE_VALID(bit6)`及可选`TARGET_RIGHT(bit7)`的`DISPERSE_PILE`，F407仅打开非目标侧并沿0.30 m镜像曲线退出，随后相机140°复审并上报`mode=35`；若无法可靠分侧，则发送不带bit6/bit7的`DISPERSE_PILE`，F407自动执行整堆退后蓄力撞分并上报`mode=34`，原地等待上位机重选目标。mode34后找到目标可直接重新APPROACH，确认找不到时才用HOLD回普通SEARCH。
...
5. `GRAB_OBJECT`：SEARCH收到1帧合法单目标便锁定类别并进入APPROACH，同类新SEQ才更新X/Y。正常靠近和中距离速度均为350 mm/s，近距有效距离≤250 mm时为125 mm/s。相机到125°立即停止平移，只原地对正到横向误差≤24 px；随后相机到140°，锁存IMU航向并以350 mm/s等待目标重新入镜。重获且误差≤96 px后进入WATCH，但继续按锁存航向以150 mm/s慢爬；上位机带STABLE的非空夹内审核一到即停车等待GRAB。从140°稳定点累计最多前进300 mm仍未确认则进入REACQ，以1°/40 ms慢抬相机到90°寻找，最终回SEARCH。NormalRun不再按250 ms帧龄自行停车，控制改变由上位机明确命令负责。
6. `RETURN_SAFE`：夹紧目标后接收带`STAGE_ONLY(bit6)`的NAV，按实时H/D到安全区入口前400 mm预备点；D=0只停车并上报`mode=10 + DISTANCE_DONE`。随后第一次ALIGN按红90°/蓝270°用IMU对正，第二次ALIGN把上位机冻结框的水平像素误差换算成一次相对角度修正；两次分别以新鲜`mode=11 + ACK`完成。视觉5秒失败时跳过第二次修正并保持第一次航向。
7. `DELIVER`：ENTER只接收围栏法向剩余距离，不再接收动态车头方向。F407保持ALIGN锁存航向，D从300 mm内按400→200 mm/s平滑减速；到约113 mm机构接触位置后忽略定位变化，以300 mm/s编码器补推200 mm，最长1200 ms。随后同时打开左右爪、摄像头转到120°并稳定300 ms，再进入`CHECK(mode15)`原地保持至少1200 ms。收到`TASK_COMPLETE`后原有`EXITSAFE(mode16)`后退0.30 m与`FACE_FIELD_CENTER(mode17)`返中流程不变。
...
当前夹爪角度为：收缩左23°/右147°，Touch左80°/右100°，Open左108°/右72°。单侧分离时释放侧完全Open，保留侧维持普通Touch：释放左侧为左108°/右100°，释放右侧为左80°/右72°。随后的YIELD按命令距离连续执行后退、向保留侧横移并向释放侧转头的镜像弧线，完成后相机到140°复审；LCD显示`ACT:YIELD CURVE/CAM/WAIT/DONE`。安装机构后必须先断开机构负载标定角度，确认不会顶死舵机。

左右归属不明时，抓取前的`DISPERSE_PILE`（不带bit6/bit7）和抓取后第一次正式`RELEASE_BOTH`共用特殊撞分支：双爪先全开让货物留在原处，以500 mm/s后退0.40 m建立加速距离；双爪回Touch并等待机构到位后，以1000 mm/s恒速定距前进0.60 m，约0.40 m处接触物块并继续推散约0.20 m；随后以500 mm/s后退0.60 m回到撞击起点，双爪重新完全打开并上报`mode=34`。此时F407保持原地等待；上位机应先重选之前的目标，找到后直接发送普通或聚集APPROACH，确认找不到时才发送HOLD回SEARCH。
