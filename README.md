...
Task模式LCD不绘制场地图、本地位置或目标距离，只保留四行核心信息：Task状态与剩余时间、上位机视觉报告中的目标X/Y坐标、红蓝方/启动区/串口状态、四类目标数量。超过250 ms没有新的有效视觉报告时X/Y显示`----`；串口超过250 ms未收到任何新帧时显示`TMO`。动态值每100 ms刷新；所有布局和文案封装在`Lcd.c`，`Robot.c`只负责传递同一份原子视觉快照并调度刷新。
