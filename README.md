...
当前只启用完整`Task`，所有测试模式和独立`CenteringTask`均关闭，并已应用`danmo-teng/shijue_fangan@c576447`提供的连续搬运F407补丁。视觉坐标为原生1280×1024、中心`(640,512)`；正常任务接收`TYPE=0x11/0x12/0x18`，发送`TYPE=0x15/0x17`。抓取命令持续重发直到F407回报夹爪闭合；返安全区和靠近中心均使用RDK发送的绝对航向+剩余距离，由本地编码器定距，不接收周期性实时位置流。详细状态机和协议见[MISSION_PROTOCOL.md](MISSION_PROTOCOL.md)。
...
`GRABBED=0`时，P6只编码当前选中的一个目标类别，不得统计整张画面的所有目标。没有目标时整个P0～P7必须为0。F407会拒绝坐标越界、bit7非0、距离有效位与距离数值矛盾以及无目标但载荷非0的报告；250 ms内没收到新的有效目标报告就停车，不会沿用旧坐标。当前上位机未测距时发送`FOUND=1、CLASS_VALID=1、DISTANCE_VALID=0、distance=0`，并在P6的普通/核心/伤员/危险对应两位字段中只置一个计数。
...
2. `START`：上电后仍依次将左爪舵机4转到23°、右爪舵机2转到147°形成安全Retract姿态；收到1帧合法配置后启动连续任务，不设置180秒倒计时。前600 mm保持舵机1为55°、夹爪收纳，以700 mm/s执行编码器定距＋IMU航向保持的直线倒退；完成后舵机1到85°、左右爪同时打开，并在剩余约1.10 m内保持地面直线、闭环转约180°。总里程最后100 mm降到160 mm/s，进入10 mm容差后制动并直接开始搜索。
3. `DISPERSE`：原有“前进0.20 m、正反各转一圈、后退0.30 m”代码仍保留，但当前`APP_ENABLE_START_SCATTER=0`，正常流程不会进入这些状态。
4. `FIND_OBJECT`：普通搜索先以摄像头120°等待700 ms，再以160 mm/s原地转满360°；没有目标就停车并快速命令舵机3到90°，稳定300 ms后再转一圈。两圈均无目标才恢复本轮起始航向，以编码器定距＋IMU航向保持前进0.80 m，再从120°开始新一轮。返中定距完成后的第一轮直接从90°开始。预计落点距场地边界不足200 mm时先改朝场地中心；旋转和前进期间持续接收新视觉帧，并用报告代次隔离旧坐标。
5. `GRAB_OBJECT`：收到合法单目标后按X修正底盘，并由舵机3根据Y执行视觉PID。目标丢失时立即停车且保持车头方向，完整等待500 ms；随后舵机3以1°/40 ms从当前角度慢扫到140°、停留300 ms、再慢扫到90°，扫描全过程持续检查新视觉帧。500 ms静止窗口完成后，任何阶段重新发现目标都会立刻回到APPROACH。仍无目标才保持90°原地转一圈；再次失败则恢复原航向，以编码器定距＋IMU航向保持前进1.00 m后回到标准SEARCH，预计越界时改朝场地中心；恢复过程最长60秒。正常接近达到140°后进入抓取观察，收到`GRAB_CONFIRMED`时左右爪同时合到Touch姿态，保留2秒完成窗口再上报闭合。
6. `RETURN_SAFE`：夹紧目标后等待上位机`TYPE=0x18`航向+剩余距离。F407用本地IMU/Location完成绝对航向对准，再锁存编码器定距；剩余300 mm内NAV从800 mm/s线性降到160 mm/s，RETURN从500 mm/s降到120 mm/s，不需要`TYPE=0x16`实时位置。定距完成后`TYPE=0x17 P0 bit5 DISTANCE_DONE=1`，供上位机可靠切换到安全区对正。
7. `DELIVER`：到达后仍执行`ALIGN_SAFE_ZONE`，所以车头会在最终30°内以120 mm/s慢速对正红方90°或蓝方270°。持续收到`ENTER_SAFE_ZONE`后左右爪同时打开，直接进入`CHECK`原地保持至少1200 ms，不再后退0.30 m或前冲0.55 m；窗口结束且收到`TASK_COMPLETE`后才后退0.45 m退出。随后执行现有`RETURN_CENTER`航向+距离，定距完成后摄像头快速到90°并直接开始低视角搜索。
...
估算底盘前进距离。正式前进轮速比例为`M1=+0.866、M2=-0.866、M3=0`，后退时M1/M2符号相反；横移比例为`M1=-0.5、M2=-0.5、M3=+1.0`，原地旋转仍为三轮同号。Task开局前600 mm直接使用`Motor_MoveDistance()`锁存编码器起点和IMU航向，完成后才展开机构；总1.70 m仍由`Location.path_mm`约束。每次定距启动前确认三轮低速稳定，再以编码器前向解算独立决定剩余距离和3 mm容差制动。导航专用`Motor_MoveDistanceLinear()`保持同一定距闭环，但在内部剩余300 mm内把NAV速度目标降到160 mm/s、RETURN降到120 mm/s，减少终点惯性前移。
