...
5. `GRAB_OBJECT`：SEARCH收到1帧合法单目标便立即以350 mm/s进入APPROACH；后续合法新SEQ持续刷新低通后的X/Y和距离。无目标帧与暂时无新SEQ都按最后合法目标计时：250 ms内保持、250～600 ms平滑降速、600 ms后停车并冻结舵机，连续1200 ms没有合法目标才进入REACQ。近距离仍按500/250 mm阈值降到225/125 mm/s；恢复流程仍为停车500 ms、慢扫140°→90°、90°转一圈和1.00 m换位。
