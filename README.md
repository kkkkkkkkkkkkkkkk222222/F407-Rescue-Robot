# 当前固件：四出发区Task与LCD场地图

当前暂时关闭Task和车辆出发动作，作为RDK X5通信联调固件运行：主函数不下发电机或舵机动作。上位机接新PCB的“串口1”，该接口使用MCU USART3的PD8 TX、PD9 RX和115200 8N1；RDK X5使用`/dev/ttyS1`，两端TX/RX交叉并共地。
...
旧Task代码仍把`TYPE=0x16`用作任务状态：状态改变时立即发送，并在任务运行期间每200 ms发送一次。当前`APP_ENABLE_TASK=0`，因此本阶段不会发出该帧。联调说明已把`0x16`预留给后续RDK融合位姿；正式接入融合位姿前，需要由上位机负责人确定新的任务状态类型，届时再统一调整，当前先不改后续流程。
...
6. 当前所有独立运动测试和`APP_ENABLE_TASK`均为0，上电只显示数据并进行UART联调，不驱动电机和舵机；后续任务流程暂不调整。

接通电机12 V前应确认三轮与编码器方向正确，并给车辆留出安全空间；只有配置成功后按下S1才开始180秒倒计时并定距前进0.7 m。

## 自主救援流程

完整流程只公开一个非阻塞入口`Task_Process(now_ms)`。TIM6每20 ms发布节拍，最低优先级PendSV执行高层状态机，电机10 ms闭环仍留在TIM6中断。当前状态为`WAIT_CONFIG`、`START`、`FIND_OBJECT`、`CRAB_OBJECT`、`RETURN_SAFE`、`DROP_OBJECT`和`STOPPED`，详细数据帧、抓取规则、LCD内容及返航放置流程以README最前面的“代码思路”为准。
...
输出位于`build/Debug/WWW.elf/.hex/.bin`。当前固件不运行车辆出发程序，也不下发舵机动作；新PCB“串口1”（USART3）独立于Task以100 Hz发送`TYPE=0x15`三路累计编码器计数。可用`tools/vision_protocol.py`验证配置、视觉报告和里程计帧。T265上位机必须使用实测轮径/每圈计数以及正确的相机安装外参；后续融合位姿和任务流程待上位机协议确定后再接入。
