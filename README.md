...
6. `RETURN_SAFE`：夹紧目标后持续接收上位机最新`TYPE=0x18`航向+剩余距离。NAV进入最后300 mm时限速约400 mm/s；首次进入D≤100 mm时，无论当前车头方向如何，F407都立即停车并按红方90°/蓝方270°完成放置区法向对正，误差≤2°后才推进最后100 mm。此后忽略上位机H变化，只保持固定方向；D重新大于150 mm解除锁定。上位机发送D=0 mm时停车，0～50 mm内保持停车。RETURN不使用该锁存。
...
Task模式LCD保留当前任务状态、上位机X/Y、导航角度、距离D、命令状态和摄像头角度。`T`表示上位机目标航向，`L`表示最后100 mm采用的固定放置区航向，`A`表示F407实际航向；`TMO/STALE/DONE`分别表示任务帧超时、余量无进展和收到D=0停车。
