...
2. `START`：Task运行后先停车，依次将左爪舵机4转到30°、右爪舵机2转到150°形成Retract收缩姿态；配置连续3帧确认成功后自动启动180秒倒计时，以850 mm/s倒车1秒退出安全区。制动后舵机1恢复85°，先将右爪舵机2转到100°，再将左爪舵机4转到80°形成Touch姿态，然后调用`Motor_TurnAngle(180.0f)`并停车等待5000 ms。
