...
由于上位机尚未完成，当前配置暂时设置为`APP_ENABLE_ROTATION_TEST=1`，`APP_ENABLE_TASK`会自动变为0，因此不会进入救援任务，也不需要发送赛前配置帧。上电时必须保持小车静止，LCD显示`IMU660RC: OK`一秒后进入`ROTATION TEST`界面。

测试步骤：

1. 第一次先架空三只轮子，确认附近无人且线缆不会被卷入；验证方向正确后再把小车放到地面。
2. LCD显示`WAIT S1`后按一次S1，程序调用`IMU_ZeroYaw()`清零相对偏航角，再执行`Motor_Move(0, 0, 164 mm/s)`原地旋转。
3. 距离90°剩余20°时，旋转轮速目标降为120 mm/s；偏航角绝对值达到88°时主动制动。这里保留2°提前量用于抵消机械惯性，实车最终角度以LCD和车体标记为准。
4. 旋转过程中再次按S1会立即取消并停车；8秒仍未达到目标会强制停车并显示`TIMEOUT`。IMU或任一电机出现故障也会立即联停。
5. LCD实时显示`STATE`、目标角、`YAW`（单位mdeg）、当前旋转速度、M1/M2/M3编码器累计值和电机故障状态。`90000 MDEG`就是90°；正负号用于判断实际旋转方向。

旋转角度、方向、快速/慢速、减速区、容差和超时时间都集中在`Main/Inc/app_config.h`的`APP_ROTATION_TEST_*`配置中。旋转测试结束、准备恢复完整任务时，只需将`APP_ENABLE_ROTATION_TEST`改为0，`APP_ENABLE_TASK`会自动恢复为1；编译期检查会阻止旋转测试与其他测试模式同时开启。
...
估算底盘前进距离，M1/M2轮速为`-0.866/+0.866`倍，M3停止。当前驶出出发区使用300 mm/s巡航；减速区取100 mm和全程一半中的较小值，进入减速区后清除巡航积分，并把速度目标从300 mm/s线性降到120 mm/s。PWM仍完全由固定45%基础值和实际编码器速度误差的PI修正得到，不再人为同步压低PWM上限，因此不会因为轮胎低于约50%无法克服静摩擦而在终点前提前停住。进入3 mm容差后主动制动。
