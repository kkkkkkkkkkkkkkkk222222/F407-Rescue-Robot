...
安全区扫障使用独立`CLEAR_SAFE_ZONE=0x13`，不会复用SEARCH/APPROACH阶段的CHANGE_LANE。它只在围栏前600 mm预备点完成第二次视觉ALIGN后接受：普通/核心把当前货物右移150 mm暂放，伤员左移150 mm暂放；随后回中抓取正前方障碍、放到相反侧、回到预备点重新夹回原货物。F407回中后保持mode39 500 ms，再进入mode23复审；合法后上报mode40，上位机必须重新执行两次ALIGN后才可ENTER。上位机走廊检测必须排除当前夹内货物，把P2/P3换算成扣除机械偏移后的实际前进距离，并使用视觉ALIGN完成后的新鲜safe_bbox而不是转向前冻结框。`RELEASE_BOTH`现在始终是真实双开/mode34；无侧DISPERSE才是20°观察/mode35。
