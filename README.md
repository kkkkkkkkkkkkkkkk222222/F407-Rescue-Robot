# 当前固件：4号位粗定位与LCD场地图演示

当前配置为`APP_ENABLE_LOCATION_DEMO=1`、`APP_ENABLE_MOTION_TEST=0`，完整救援任务暂时关闭。上电静止校准IMU并显示`IMU660RC: OK`一秒后，程序假定小车位于4号出发区中心，舵机方向朝场外，先以300 mm/s倒车3秒，越过左侧三根减速带进入场内后停车；LCD每100 ms显示估计坐标、航向和场地俯视图。

## 定位坐标系与算法

- 以3000×3000 mm场地中心为原点，向图纸右侧为`+X`，向图纸上方为`+Y`，航向角从`+X`逆时针增加。4号出发区中心约为`(1350,-1350) mm`，车头朝场外时初始航向为0°；倒车3秒的理想终点约为`(450,-1350) mm`，实际LCD位置以编码器测量为准。
- `Location.c`每10 ms取得同一周期的三轮编码器增量。根据实车映射`M1=v2、M2=v1、M3=v3`，先求车体前向位移`F=(M3-M1)/sqrt(3)`和物理左向位移`L=(M1+M3-2*M2)/3`，再用本周期IMU中间航向旋转到场地坐标系并积分。三轮共同的原地旋转分量会在这两个式子中抵消，航向完全采用静止校准后的陀螺仪积分值。
...
- LCD地图按赛事图绘制四个300×300 mm出发区、上下两个约660×360 mm安全区、中心轴线，以及四个出发区两条场内方向上的全部减速带；黄色点代表当前车体中心，白线代表车头方向，绿色轨迹点保留最近48个实测位置。LCD有矩形填充和文字接口，本次补充了内部短线绘制，因此不需要更换屏幕驱动。
...
测试结束后将`APP_ENABLE_LOCATION_DEMO`改为0，`APP_ENABLE_TASK`会自动恢复为1。以后收到赛前配置的出发区编号时，可直接以`Location_Reset((LocationStart)start_zone)`建立对应的初始位置。
...
6. 当前配置为`APP_ENABLE_LOCATION_DEMO=1`、`APP_ENABLE_TASK=0`。上电后执行4号位定位演示；恢复任务时关闭定位演示，`APP_ENABLE_TASK`会自动变为1。
...
输出位于`build/Debug/WWW.elf/.hex/.bin`。当前固件上电后从4号位倒车穿过左侧减速带并显示定位地图；先悬空确认倒车时M1正转、M2基本不转、M3反转，同时LCD黄色点向左移动，再落地低速测试。恢复任务后再用`tools/vision_protocol.py`验证三帧配置、导航超时和抓取流程。S4开闭角、方向、堵转门限、实际轮径、每圈计数和PID参数都必须结合实车数据继续整定。
