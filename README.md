...
2. LCD显示`WAIT S1`后按一次S1，程序先调用`Motor_Move(0, 0, 328)`，通过三轮运动学和编码器速度PID原地旋转3秒。
3. 到3秒时直接切换为`Motor_Move(600, 0, 0)`，以固定朝向照片右下方的舵机为车头：M1=+300 mm/s、M2=+300 mm/s、M3=-600 mm/s，三轮合成直线运动；持续3秒后调用`Motor_Stop()`制动停车并显示`DONE`。当前旋转和前进速度分别是最初164和300 mm/s测试值的两倍。
4. 测试过程中再次按S1会立即取消并停车；任何电机出现反向或堵转故障也会三轮联停。完成后可再次按S1重复整套测试。
5. LCD实时显示`STATE`、当前`STEP`、阶段时间、目标速度、M1/M2/M3编码器累计值和电机故障状态。

旋转/直行时间和速度集中在`Main/Inc/app_config.h`的`APP_MOTION_TEST_*`配置中。测试结束、准备恢复完整任务时，只需将`APP_ENABLE_MOTION_TEST`改为0，`APP_ENABLE_TASK`会自动恢复为1。
...
| 功能 | 引脚/外设 | 参数 |
| --- | --- | --- |
| M1 | PA2/PA3 TIM5 CH3/CH4；PE9/PE11 TIM1编码器 | 20 kHz，固定朝向照片下方轮 |
| M2 | PE5/PE6 TIM9 CH1/CH2；PA6/PA7 TIM3编码器 | 20 kHz，固定朝向照片左上轮 |
| M3 | PB10/PB11 TIM2 CH3/CH4；PD12/PD13 TIM4编码器 | 20 kHz，固定朝向照片右上轮 |
...
正数朝固定朝向照片右下方的舵机前进、负数后退，单位为米。它记录三轮的64位起点，用：

```text
forward_distance = (M1_distance + M2_distance - 2 * M3_distance) / 3
```

估算底盘前进距离。固定朝向照片中，前进轴指向右下方舵机，三轮速度比例为`M1=+0.5、M2=+0.5、M3=-1.0`；横移比例为`M1=+0.866、M2=-0.866、M3=0`，原地旋转仍为三轮同号。当前驶出出发区使用300 mm/s巡航；减速区取100 mm和全程一半中的较小值，进入减速区后清除巡航积分，并把速度目标从300 mm/s线性降到120 mm/s。PWM仍完全由固定45%基础值和实际编码器速度误差的PI修正得到，不再人为同步压低PWM上限，因此不会因为轮胎低于约50%无法克服静摩擦而在终点前提前停住。进入3 mm容差后主动制动。
