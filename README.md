...
`NormalRun`启用完整救援Task，并已对照上位机`codex/gamepad-teleop@e5fb078`的抓前/抓后双重审核、分离、STAGE/ALIGN/ENTER和返中命令。正式投送先到围栏前600 mm预备点并完成两次ALIGN；ENTER由F407本地执行前400 mm接近和后200 mm补推，总目标600 mm，停车、张爪、相机120°稳定后才上报mode15。ALIGN首次进入1.5°、稳定期使用4°迟滞，避免IMU抖动使STAGE→ALIGN长期无法完成。
...
开局藏堆沿用`STASH_NONEMPTY`：只要INITIAL_STASH审核非空，抓前和mode23抓后均以3个不同audit_id累计，不套用正式投送的类别、数量、危险/未知或伤员混装限制；合法后进入mode22并将下一条NAV锁存为`route_to_stash`，到藏点可直接双开，且不会消耗“首件绿色”资格。

普通/聚集目标在仍依赖目标坐标的APPROACH跟踪与水平对正阶段收到HOLD会立即停车；500 ms内没有新的APPROACH_TARGET则进入mode24，再静止500 ms回mode3重新SEARCH，消除上位机持续HOLD与F407永久mode20互等。相机已经开始到140°、140°稳定或聚集夹内审核后属于本地收尾阶段，不再依赖目标坐标，HOLD不会打断。

安全区航向不再区分首趟：上位机与F407统一使用红方90°、蓝方270°的绝对ALIGN，冻结框视觉相对修正只叠加一次并锁存到ENTER。

mode15投送确认阶段的本地等待由1200 ms统一为1000 ms，与上位机单次1秒视觉观察窗口一致；该改动只缩短TASK_COMPLETE被执行前的静止等待，不改变ENTER距离、张爪、相机120°、退出或返中动作。

安全区扫障对齐上位机`4115aea`：`CLEAR_SAFE_ZONE=0x13`的视觉请求为P2/P3=0、P4/P5=±200；转向90°前进200 mm放置，不再横移。42/43取障，44/45找回原物资（累计前进最多200 mm），两次取物后均按位置/航向轨迹逆序倒退回S。找回合法GRAB后经39→23→40再重新ALIGN；失败张爪退回S后报46，等待RETURN_CENTER。无障碍直接ENTER，旧固定距离80～600 mm配±150仅保留兼容。普通释放/20°观察不变。协议冲突与现场注意事项见`docs/UPPER_4115AEA_ALIGNMENT.md`。

定位距3 m×3 m地图任一边≤300 mm时，下位机自主取消当前动作、张爪并转向场地中心，随后以300 mm/s驶入距边≥400 mm的安全带，整个过程上报mode41，完成后才回mode3重新SEARCH；仅ENTER主动推送阶段放宽到距边50 mm，避免定位进入安全区时误触发。SEARCH完成120°和90°两圈后不会原地静止，而会以150 mm/s低速补扫，防止上位机T265少累计几度后无法发送RETURN。
...
1. 第一次正式投送必须恰好1件GREEN；开局INITIAL_STASH独立按任意非空处理，不算正式投送。
2. 首次正式投送后，普通或核心仍严格单件运输，伤员单件；多个普通、多个核心、普通+核心和MIXED_MATERIAL均非法，完整ROI数量不得截断。
...
5. `GRAB_OBJECT`：相机140°后进入mode21审核并以180 mm/s按同目标共享预算最多慢爬300 mm。3个不同audit_id语义一致且合法后置AUDIT_VALID并接受GRAB；含核心/mixed先前进50 mm再合爪。合爪完成进入mode23，保持140°、GRIPPER_CLOSED和CLAW_VISIBLE，清除旧审核后重新累计3帧；合法置AUDIT_VALID并进入mode22，mode22才接受NAV。抓后空爪回SEARCH，抓后非法允许释放或分离并继续复审。
...
7. `DELIVER`：ENTER要求`DRIVE_STRAIGHT=1、DISTANCE_VALID=0、P2..P5=0`，视觉ALIGN成功时置bit6且P6/P7=0，定位降级时置`USE_FINAL_HEADING`并发送红9000/蓝27000。F407保持ALIGN锁存航向，在第一次有效LocationPose只锁存一次编码器起点；重复ENTER只ACK。前400 mm按400→200 mm/s接近，随后以330 mm/s补推250 mm，编码器总目标650 mm，累计650 mm或末段有效推送1500 ms任一满足即结束推进，PAUSE时间不计入末段计时；仅固定距离ENTER豁免地图边界恢复。随后停车、双爪全开、相机120°稳定300 ms并进入mode15。收到`TASK_COMPLETE`后执行mode16后退0.30 m，再进入mode17返中。
...
当前夹爪角度为：收缩左23°/右147°，Touch左78°/右102°，Open左108°/右72°。所有聚集物带侧曲线均使用15°保持：释放左侧为左108°/右117°，释放右侧为左63°/右72°。普通单侧释放使用25°强保持：释放左侧为左108°/右127°，释放右侧为左53°/右72°。安装机构后必须先断开机构负载标定角度，确认不会顶死舵机。

左右归属不明时不再撞击或双开整堆。只有无`SIDE_VALID`的`DISPERSE_PILE`会执行20°原地观察转向；`RELEASE_BOTH`始终是真实双开并结束于mode34。观察时相机保持140°并稳定300 ms，随后上报`mode=35`等待新的夹爪ROI审核。观察之后及任何后续带侧曲线仍统一使用15°保持，不升级为25°。稳定空爪审核会清除批次、完全张爪并回SEARCH；25°只用于普通单侧释放。
