...
对齐4115aea：首件恰好1件绿色；首件正式投送完成后，F407允许总数1～2件普通、核心或`mixed_material`组合进入GRAB/NAV。危险、未知、伤员混装及超过2件不能置AUDIT_VALID，必须释放、分离或恢复；原始总数量不得截断。INITIAL_STASH优先按非空处理。
...
- `CLEAR_SAFE_ZONE=0x13`视觉流程P2/P3=0，P4/P5=±200表示转90°后前进200 mm暂放原物资，P6/P7=0；旧80～600 mm距离形式仍配±150横移，仅保留兼容。新视觉±150拒绝。无障碍时不发0x13。完整44/45/46握手见`UPPER_4115AEA_ALIGNMENT.md`。
...
- 扫障GRAB后F407回mode39，沿记录的位置/航向节点逆序倒退回S；相反侧转90°前进200 mm放障、张爪完成后倒退回S，再转180°朝原物资。44/45找回共享200 mm前进预算，45用普通正式审核，合法等待GRAB；39合爪倒退回S恢复H→23→40。失败张爪倒退到S恢复H后46接受RETURN_CENTER进17。不能把累计path作为直线回程距离，也不再掉头前进回S。
