...
- 暂时丢帧时可HOLD停车，但不能把未完成动作直接标记完成；恢复时继续重复原命令和递增SEQ。
...
- 不要让SEARCH计时、目标确认计时和F407本地90°/120°扫描圈数共同决定同一次状态跳转。上位机负责目标选择和策略，F407负责扫描动作；上位机确认候选后持续发送APPROACH，HOLD仅作为可恢复安全停车。
