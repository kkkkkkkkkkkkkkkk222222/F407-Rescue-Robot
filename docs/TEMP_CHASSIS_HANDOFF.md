...
- 动态载荷必须满足距离0～5000 mm、航向0～359.99°、直行/航向/距离有效位齐全。距离≤2 mm停车，停车后必须重新大于50 mm才恢复；任务命令超过250 ms立即停车；车辆前进时距离载荷连续500 ms没有至少2 mm变化会锁定等待，直到上位机距离重新变化，避免100 Hz心跳掩盖冻结的T265/融合位置。
...
- NAV接近放置区时仍由上位机在约250～300 mm处把heading切换为放置区方向（红90°/蓝270°）；F407检测到最新航向后先停车对正，进入最后300 mm时将NAV速度限制约400 mm/s，再走完余量并在D≤2 mm停车。F407不自行猜测红/蓝方向，最终航向仍来自上位机。由于20 ms任务周期和电机制动存在离散误差，2 mm是命令门限，不保证物理车体误差恰为2 mm。
...
- NAV首次进入`D≤100 mm`时并不会无条件锁定；必须已经完成上位机最终航向对正且误差≤2°，随后记录F407当前Location/IMU航向。最后100 mm忽略上位机后续heading变化，禁止7°原地重新对正，但仍以锁存角做小幅IMU直线保持。
- 上位机距离因定位回跳重新大于150 mm时解除锁定，恢复动态航向控制；ENTER、RETURN或新一轮NAV同样自动清除锁定。该迟滞避免100 mm边界附近来回切换。
- LCD未锁定时显示`T:目标 A:实际 D:余量`，锁定后显示`L:锁存 A:实际 D:余量`。RETURN_CENTER不使用近距离锁存，仍持续跟随上位机返中航向。
