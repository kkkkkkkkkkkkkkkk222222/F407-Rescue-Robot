...
1. 架空车轮，确认收到配置后双爪同时打开，底盘在倒车1.70 m期间同时保持地面直线路径并闭环转约180°；只在最后100 mm减速，到达后不再单独转向或等待5秒。
...
- `TASK_START`删除原来的“到达后制动等待→Touch→单独转180°→等待5秒”子步骤。现在舵机1到85°、双爪打开、倒车1.70 m和转向约180°同时进行；底盘使用`Motor_MoveSpin()`保持初始地面后退直线，转向速度由IMU航向误差闭环给出。主体请求850 mm/s，最后100 mm以160 mm/s收尾。
