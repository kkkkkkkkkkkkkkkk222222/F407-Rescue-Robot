...
{
  motor_set_speed_target( 0.5f * speed_mm_s, 1U);
  motor_set_speed_target( 0.5f * speed_mm_s, 2U);
  motor_set_speed_target(-speed_mm_s, 3U);
}
...
static void motor_update_distance_move(const EncoderStatus encoder[MOTOR_COUNT])
{
  ...
  const float forward_mm = (m1_mm + m2_mm - 2.0f * m3_mm) / 3.0f;
...
void Motor_Move(float forward_mm_s, float lateral_mm_s, float rotate_mm_s)
{
  ...
  float wheel[3] = {
     0.5f * forward_mm_s + 0.8660254f * lateral_mm_s + rotate_mm_s,
     0.5f * forward_mm_s - 0.8660254f * lateral_mm_s + rotate_mm_s,
    -forward_mm_s + rotate_mm_s
  };
