...
5. 普通`SEARCH`先把摄像头置120°并以160 mm/s原地旋转一整圈；没有目标时停车，直接把舵机3快速命令到90°，稳定300 ms后再旋转一整圈。两圈都没有目标才恢复本轮起始航向，以编码器定距和IMU航向保持前进0.80 m，然后从120°重新开始。返中定距完成后的第一轮搜索例外，直接从90°开始。移动落点距场地边界不足200 mm时改为朝场地中心移动。进入每轮搜索时记录视觉报告本地代次，只有随后收到的新报告才允许X/Y触发目标锁定；旋转和前进期间均持续检查新视觉帧。
