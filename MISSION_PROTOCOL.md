...
本固件以`danmo-teng/shijue_fangan`的`codex/gamepad-teleop@b0f954f`为对齐基线。正式运输首件恰好1件GREEN，之后普通/核心合计1～2件、伤员单独1件；INITIAL_STASH仍优先按任意非空处理。运行模式为`APP_ENABLE_TASK=1`，RDK X5与F407使用USART3（PD8 TX、PD9 RX）、115200 8N1、3.3 V TTL和公共15字节帧。扫障新握手及剩余协作风险见`docs/UPPER_4115AEA_ALIGNMENT.md`。 聚集130°对正累计360°刷新及mode24/HOLD握手见`docs/UPPER_2FD9F75_CLUSTER_RECOVERY.md`。 当前必须使用1D/18与1E/17同SEQ帧对，详见`docs/UPPER_B0F954F_CONTEXT_READY.md`；旧裸18除ABORT外不再接受。 限推300 mm、失败退让、三次松爪和S1抬爪退出的最新行为见`docs/CAPTURE_300MM_SEPARATION_EXIT_HANDOFF.md`，上位机需要同步适配。 第三次分离复审已收齐但仍非法时，单独走张爪→退300 mm→mode47等待RETURN→mode17返中→mode3，详见`docs/UPPER_THIRD_SEPARATION_RETURN_REQUEST.md`；其他既有放弃流程不变。
...
3. 用编码器累计路程倒车1.70 m。前600 mm由`Motor_MoveDistance(-0.60 m, 1000 mm/s)`直接锁存两路前向编码器起点，以3 mm容差做定距闭环并用IMU保持启动航向；舵机1保持55°、左右爪保持Retract，确保机构不碰障碍区。完成600 mm后舵机1才到85°、左右爪同时打开到左108°/右72°；剩余约1.10 m以最高1000 mm/s使用`Motor_MoveSpin()`保持地面直线路径，同时根据IMU航向误差闭环转向约180°。总里程最后100 mm降到160 mm/s，进入10 mm容差后制动。
...
8. 普通mode21及聚集mode38从第一次140°夹内慢爬开始共享同目标300 mm前进额度，APP/审核更新和短暂后退不重置或返还。到限停车审核；合法等待GRAB，无合格物资或4秒内无有效分离方案时在mode24完成张爪→后退300 mm→左转90°→mode3，不再直接重靠近旧目标。
...
11. 上位机随后发送第一次`ALIGN_SAFE_ZONE`：P6/P7为红方90°或蓝方270°，F407用IMU原地对正，并确保摄像头120°命令后至少稳定300 ms。首次进入3°，稳定期使用6°迟滞窗口，避免IMU小幅波动反复重启ALIGN；完成后上报`mode=11`。上位机连续3帧冻结安全区框后发送第二次带`VISUAL_CORRECTION_VALID(bit6)`的ALIGN；P2/P3为有符号水平像素误差而不是角度。F407按可标定焦距把像素误差转换为一次相对转角，最大限制±15°，完成后再次上报新鲜`mode=11`并锁存最终航向。视觉5秒失败时，上位机跳过第二次ALIGN，直接沿第一次90°/270°结果推进。
...
15. 收到`TASK_COMPLETE`后进入mode16：S1抬到开局55°并等1秒，再以400 mm/s后退0.30 m；后退完成停车，S1回85°并等1秒，再进入mode17使用RETURN H/D返中。S3保持120°；RETURN提前到达只缓存/ACK，不打断升降或重置退出行程。D=0后mode3重新搜索。
...
- 普通目标在140°稳定500 ms后进入mode21并以180 mm/s按同目标预算最多慢爬300 mm。所有审核统一要求3个不同audit_id语义一致，第三帧后合法才置AUDIT_VALID。mode23表示合爪后复审，mode22表示抓后审核合法并允许NAV。
...
- 非法物资分离仍使用140°全夹爪ROI。Touch为S4=76°/S2=104°；同一保留侧第一次选用15°曲线或25°单侧释放基准，第二次向张开方向松10°、第三次松20°，每次动作后复审，第三次仍非法则退让重新找目标。20°无侧观察不变，详细角度及上位机配套要求见最新限推交接说明。
