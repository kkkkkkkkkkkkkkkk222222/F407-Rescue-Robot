...
- 非法组合优先单侧分离。F407只接受与最近稳定审核一致的释放侧：左/右释放要求对应侧计数非零；`RELEASE_LEFT`为左108°/右110°，`RELEASE_RIGHT`为左70°/右72°。若上位机无法判断物资分别位于哪侧，第一次正式审核发送`RELEASE_BOTH`：F407会按约定只开左爪到108°、右爪保持正常Touch 100°，随后由`YIELD_BACKOFF`后退分开并把相机移到140°复审。复审合法后发送`GRAB_CONFIRMED`，双爪回左80°/右100°；复审仍非法时再次发送`RELEASE_BOTH`才会真正双爪全开。临时藏堆的`RELEASE_BOTH`始终是双爪全开，不套用该不明侧约定。
...
- 普通`YIELD_BACKOFF`使用750 mm/s编码器定距和IMU航向保持；单侧或不明侧分离后，最初100 mm为450 mm/s，后段恢复750 mm/s，退让后舵机3到140°并稳定300 ms才报告`YIELD_DONE`。`DISPERSE_PILE`只在空爪且未等待复审时执行：先把双爪合到Touch，以700 mm/s沿车头直冲0.50 m；再双爪全开，依次相对摆头`左45°、右90°、左105°、右120°、左60°`，等价于相对初始航向到`+45°、-45°、+60°、-60°、0°`；最后以650 mm/s倒退0.50 m回到原路径。动作完成才报告mode35，专用保护上限15秒。
