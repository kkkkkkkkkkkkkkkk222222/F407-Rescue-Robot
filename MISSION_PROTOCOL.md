...
7. `mode=21`保持锁存航向并以180 mm/s慢爬，最多500 mm。所有最终允许GRAB的审核都要求两个不同`audit_id`且物资集合一致；左右类别及对应数量整体交换仍视为一致。第一张合法后的一张瞬时非法审核由上下位机共同忽略，不覆盖合法候选；连续第二张非法才允许重新分离。危险/未知/混装标志或持续的真实类别数量变化仍会切换审核基线。
