...
本固件已接入`danmo-teng/shijue_fangan`的`codex/gamepad-teleop@2183fdf`完整比赛流程命令，并扩展首件绿色轻撞和两圈无目标返中语义。运行模式为`APP_ENABLE_TASK=1`，RDK X5与F407使用USART3（PD8 TX、PD9 RX）、115200 8N1、3.3 V TTL和公共15字节帧。
...
5. 每次进入`SEARCH`先把舵机3移到120°并旋转一整圈，再切到90°稳定300 ms后旋转一整圈。两层共约720°均无目标后F407保持`mode=3`并停车在`SEARCH_WAIT_RETURN`，不再开始第三圈；上位机此时发送现有`RETURN_CENTER H/D`，F407进入mode17返回场地中心，D=0后重新从120°开始。任一扫描阶段仍可由普通/聚集APPROACH接管。
...
7. `mode=21`保持锁存航向并以180 mm/s慢爬，最多500 mm。所有最终允许GRAB的审核都要求两个不同`audit_id`且内容一致：普通抓取第一帧可非STABLE、第二帧置STABLE；聚集和分离复审中的合法单件同样累计两张新帧。无效审核仍允许一张显式STABLE后决定分侧。同一视觉帧重复转发保持audit_id，不增加F407计数。
...
12. `ENTER_SAFE_ZONE`必须置`DRIVE_STRAIGHT`、清除`DISTANCE_VALID`且P2～P5为0；视觉ALIGN成功时bit6=1、清除`USE_FINAL_HEADING`且P6/P7=0，定位降级时bit6=0、置`USE_FINAL_HEADING`并发送红9000/蓝27000。F407本地编码器总目标为600 mm：前400 mm使用既有接近减速，随后以300 mm/s最终补推200 mm并保留1200 ms接触保护；停车、双爪全开且相机120°稳定300 ms后才上报mode15。
...
- `HOLD`是高层心跳：SEARCH的120°/90°两圈尚未完成时继续扫描；进入`SEARCH_WAIT_RETURN`后保持停车并等待合法RETURN_CENTER。需要冻结其他阶段时使用PAUSE。
...
- 普通目标在140°稳定500 ms后直接进入mode21、置`CLAW_VISIBLE=1`并以180 mm/s最多慢爬500 mm，不再等待原track重新出现。普通以及聚集/分离后的最终GRAB都要求两个不同audit_id且内容一致；无效审核的选侧仍允许单个显式STABLE新帧。
...
- 首件绿色若在140°ROI中与其他物资挤在中心且曲线无法取出，上位机可发送`DISPERSE_PILE + FIRST_GREEN_BUMP(bit5)`，不得同时置SIDE_VALID/TARGET_RIGHT。F407仅在首件尚未完成、审核至少2件且含绿色时接受，并且每轮任务最多一次：后退0.10 m→Touch闭爪→前进0.20 m→后退0.10 m→双爪全开→直接回SEARCH。该动作不改变无侧DISPERSE的12°观察语义。
...
- `YIELD_BACKOFF`只在F407已确认完成普通单侧释放且`cargo_recheck_pending=1`时接受，该路径保持25°。聚集APPROACH保持双爪Open，mode38审核后进入mode37；所有带侧DISPERSE曲线都使用15°保持。不带SIDE_VALID只执行12°观察并以mode35请求新审核。
