...
6. 收到原生1280×1024目标坐标后进入`APPROACH`。新目标坐标先以0.35权重低通滤波；摄像头PID按真实帧间隔计算，最大允许300 ms，并以30°/s限制舵机命令变化。上位机明确发送新的无目标/非法单目标报告时，F407立即停车进入原500 ms恢复流程；仅仅没有新SEQ时不改变舵机角度，也不立即进入REACQ：250 ms内保持运动，250～600 ms线性降低前进和转向，600 ms后停车等待，连续1200 ms仍无新目标帧才判定丢失。恢复流程仍为当前角度慢扫到140°、再到90°、90°转一圈和1.00 m换位。
