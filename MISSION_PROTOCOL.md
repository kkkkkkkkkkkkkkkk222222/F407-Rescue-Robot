...
5. 普通`SEARCH`先把摄像头置120°并以160 mm/s原地旋转一整圈；没有目标时停车，直接把舵机3快速命令到90°，稳定300 ms后再旋转一整圈。每个视角必须在本圈收到至少3帧不同SEQ的新视觉报告，才允许判定本圈无目标；不足3帧就保持当前角度再转一圈，不会因上位机低帧率贸然换角度或前进。两种视角均确认无目标后才恢复起始航向并前进0.80 m。返中完成后的第一轮直接从90°开始。
6. 收到1帧原生1280×1024合法单目标坐标后立即进入`APPROACH`，远距离以350 mm/s沿该目标方向前进。后续每个合法新SEQ刷新X/Y、距离和丢失计时；新坐标以0.35权重低通滤波，摄像头PID最大帧间隔300 ms并以30°/s限制舵机变化。无论是无目标报告还是暂时没有新SEQ，只要尚未连续丢失1200 ms就保持同一目标：250 ms内保持，250～600 ms线性降低前进和转向，600 ms后停车且冻结舵机；期间合法目标恢复便立即继续APPROACH。只有连续1200 ms没有合法单目标才进入500 ms重捕获流程。
...
11. 上位机发送`ALIGN_SAFE_ZONE`后，F407仍转到命令给出的最终航向；这就是车辆到达安全区后出现慢速旋转的来源。上位机和F407现在统一以航向误差≤8°为对正完成，收到`ENTER_SAFE_ZONE`后即可张爪；误差重新扩大到≥12°才撤销完成状态并再次对正，避免阈值附近抖动。随后直接进入`RAM_VERIFY/CHECK`原地停车，不再后退0.30 m或前冲0.55 m。
...
`P0=COMMAND、P1=FLAGS、P2/P3=DISTANCE_mm、P4/P5=0、P6/P7=HEADING_cdeg`。距离为非负int16大端，表示本段开始时上位机计算的剩余行驶距离；它不是逐帧变化的实时余量。航向范围0～35999，单位0.01°。FLAGS bit0必须为1，bit1要求直行，bit2使用航向，bit3表示红方，bit4表示距离有效。定距启动后F407锁存第一帧距离并以自身编码器递减余量，后续重复帧只维持看门狗和ACK，不覆盖已经执行的路程。
